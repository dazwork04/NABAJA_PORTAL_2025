$(window).load(function() {
    ...
    $('input[name=txtDocEntryApp]').trigger('keyup');
...
function getDocCur(selDocCur)
{
	...
	if(x == 'C') {
		...
		$('select[name=selDocCur]').load('../../proc/views/PO/vw_doccur.php?CompanyDb=' + CompanyDb, function () {
...
$(document).ready(function() {
	...
	function view_po(docentry) 
	{
		...
		{
			var url = "PO.php?DocEntry="+ docentry;
	...
	$('#window-title').text('Purchase Order');
	...
	//Initialize Title
	$('#mod-title').text('Purchase Order');
	//End Initialize Title

	
    //Load Grpo Series 
    $('#btnSeries').html('Loading...')
    $('#SeriesList').load('../../proc/views/PO/vw_series.php?objtype=22',function(){
    ...
    $('select[name=txtPayment]').load('../../proc/views/PO/vw_paymentterms.php');
	
    $('select[name=txtSalesEmployee]').html('<option>Loading...</option>');
    $('select[name=txtSalesEmployee]').load('../../proc/views/PO/vw_salesemployee.php');

    $('#OwnerModal').on('shown.bs.modal', function () {

        $('#OwnerCont').html('<p class="text-center"><img src=../../img/ajax-loader.gif /> Loading...</p>');
        $('#OwnerCont').load('../../proc/views/PO/vw_employee.php?CardType=C', function () {
            //Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/PO/vw_employee-load.php',
    ...
    $(document.body).on('keyup', 'input[name=OwnerSearch]', function () {
        ...
        $('#OwnerCont table tbody').load('../../proc/views/PO/vw_employee-load.php?srchval=' + encodeURI(searchVal));
    })
    //End Search Owner
    ...
    $(document.body).on("dp.change","#txtPostingDateCont", function(e) {
    	...
    	$('input[name=txtDeliveryDate]').val(addDays(dateselected,3));
	...
	$(document.body).on('change','select[name=cmbServiceType]',function(){
		...
		if(servicetype == 'I'){
			...
			$('#ModDetails').load('../../forms/PO/PO-details.php?servicetype=I',function(){
			...
			$('#ModDetails').load('../../forms/PO/PO-details.php?servicetype=S',function(){
				//Clear value Total Before Discount
				$('input[name=TotBefDisc]').val('');
				$('input[name=TotBefDisc]').trigger('keyup');
				//End Clear value Total Before Discount
	...
	$(document.body).on('click','#btnAddRow',function(){
		...
		if(servicetype == 'I'){
			//Item Type
			$(this).load('../../forms/PO/PO-details-row.php?servicetype=I',function(result){
			...
			//Service Type
			$(this).load('../../forms/PO/PO-details-row.php?servicetype=S',function(result){
				$('#tblDetails tbody').append(result);
	...
	$(document.body).on('click','#btnFreeText',function(){
		...
		if(servicetype == 'I'){
			//Item Type
			$(this).load('../../forms/PO/PO-details-row.php?servicetype=I&freetext=1',function(result){
	...
	$(document.body).on('click','#btnDelRow',function()
	{
		...
		var deleterow = $('.selected-det').find('input.deleterow').val();
		...
		else
		{
			...
			$.ajax({
				type: 'POST',
				url: '../../proc/exec/po/exec-delete-po.php',
				data: 
				{
	...
	$('#ItemModal').on('shown.bs.modal',function(){
		...
		$('#ItemCont').load('../../proc/views/PO/vw_itemlist.php',function(){
			//Add Scroll Function 
		        ...
		        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/PO/vw_itemlist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$('#ItemModal').keydown(function(e) {
		    switch(e.which) {
		    	case 13: //Enter
		    		$('#tblItem tbody > tr:first').trigger('dblclick');
		    	break;
		        case 37: // left
		        break;
	...
	$(document.body).on('dblclick','#tblItem tbody > tr',function(){
		...
		$.ajax({
		type: "POST",
		url: "../../proc/views/PO/vw_pricelist.php",
		data: form_data,
		success: function(html)
			{
				result = html;
		...
		$.ajax({
		type: "POST",
		url: "../../proc/views/PO/vw_poglaccount.php",
		data: form_data,
		success: function(html)
			{
				result = html;
				if(result != '')
				{
					$('.selected-det').find('input.glaccount').val(result);
				}
			}
		});
	...
	$(document.body).on('keyup','input[name=ItemSearch]',function(){
        ...
        $('#ItemCont table tbody').load('../../proc/views/PO/vw_itemlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Item
	...
	$('#WhsModal').on('shown.bs.modal',function(){
		...
		$('#WhsCont').load('../../proc/views/PO/vw_whslist.php',function(){
	...
	$(document.body).on('keyup','input[name=WhsSearch]',function(){
        ...
        $('#WhsCont table tbody').load('../../proc/views/PO/vw_whslist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Whs
	...
	$(document.body).on('keyup','input[name=EmployeeSearch]',function(){
        ...
        $('#EmployeeCont table tbody').load('../../proc/views/PO/vw_emplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Employee
	...
	$('#AcctModal').on('shown.bs.modal',function(){
		...
		$('#AcctCont').load('../../proc/views/PO/vw_acctlist.php',function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/PO/vw_acctlist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('keyup','input[name=AcctSearch]',function(){
        ...
        $('#AcctCont table tbody').load('../../proc/views/PO/vw_acctlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Acct
	...
	$('#InvDataModal').on('shown.bs.modal',function(){
		...
		if(itemcode != ''){
			...
			$('#InvDataCont').load('../../proc/views/PO/vw_invdatalist.php?itemcode=' + itemcode,function(){
	...
	$('#DocumentModal').on('shown.bs.modal',function(){
		...
		$('#DocumentCont').load('../../proc/views/PO/vw_doclist.php?servicetype=' + encodeURI(servicetype),function(){
			//Add Scroll Function 
				...
				if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) 
				{
                    if ($(this).scrollTop() > 0) 
					{
                        ...
                        if (ctr == 0) 
						{
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/PO/vw_doclist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) 
	...
	$(document.body).on('keyup','input[name=DocumentSearch]',function(){
        ...
        $('#DocumentCont table tbody').load('../../proc/views/PO/vw_doclist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Document
	...
	$(document.body).on('keyup','input[name=txtDocEntry]',function(){
		...
		$.getJSON('../../proc/views/PO/vw_getdocumentdata.php?docentry=' + docentry, function(data) {
            /* data will hold the php array as a javascript object */
	            ...
	            $.each(data, function(key, val) {
	            	...
	            	if(val.DocStatus == 'C'){
						...
						$('input[name=txtDocRef]').val(val.U_POref).prop('disabled',true);
						$('input[name=txtPRRef]').val(val.U_PRref).prop('disabled',true);
						...
						$('input[name=txtListNum]').val(val.ListNum).prop('disabled',true);
						$('input[name=txtDocRef]').val(val.U_POref).prop('disabled',false);
						$('input[name=txtPRRef]').val(val.U_PRref).prop('disabled',true);
	...
	$('#BPModal').on('shown.bs.modal',function(){
		...
		$('#BPCont').load('../../proc/views/PO/vw_bplist.php?CardType=S',function(){
			//Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/PO/vw_bplist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$(document.body).on('dblclick','#tblBP tbody > tr',function(){
		...
		if(Currency == '##') {
			...
			$('select[name=selDocCur]').html('<option>Loading...</option>');
			$('select[name=selDocCur]').load('../../proc/views/PO/vw_doccur.php', function () {
		...
		$('textarea[name=txtShipTo]').load('../../proc/views/PO/vw_shipto.php?cardcode=' + encodeURI(BPCode));
		
		//Load Bill To
		$('textarea[name=txtBillTo]').html('Loading...');
		$('textarea[name=txtBillTo]').load('../../proc/views/PO/vw_billto.php?cardcode=' + encodeURI(BPCode));
	...
	$(document.body).on('keyup','input[name=BPSearch]',function(){
        ...
        $('#BPCont table tbody').load('../../proc/views/PO/vw_bplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search BP
    ...
    $(document.body).on('keyup', 'input[name=txtBaseEntry]', function () {
        ...
        if (basentry != '') {
            //Show Base Document
            //Get PR data using JSON
            $.getJSON('../../proc/views/PO/vw_getprdata.php?docentry=' + basentry, function (data) 
			{
                $.each(data, function (key, val) 
				{
                    if (val.DocStatus == 'C') 
					{
                        ...
                        $('textarea[name=txtRemarksF]').val(val.Remarks + ' PR#' + val.DocNum + '.').prop('disabled', true);
						$('select[name=cmbServiceType]').prop('disabled', true);
					...
					else 
					{
                        ...
                        $('textarea[name=txtRemarksF]').val(val.Remarks + ' PR#' + val.DocNum + '.').prop('disabled', false);
						$('select[name=cmbServiceType]').prop('disabled', true);
	...
	//Load Base Document
    $(document.body).on('keyup', 'input[name=txtDocEntryApp]', function () {
        var basentry = $(this).val();
		
		if(basentry != ''){
			//Show Base Document
			//Get PR data using JSON
			$.getJSON('../../proc/views/PO/vw_getpodata.php?docentry=' + basentry, function(data) {
	            /* data will hold the php array as a javascript object */
           		...
           		$.each(data, function(key, val) {
	            	...
	            	if(val.DocStatus == 'C'){
		            	...
		            	$('#btnSave').addClass('hidden');
						$('#btnCancelDoc').addClass('hidden');
		            	$('#btnCloseDoc').addClass('hidden');
						disablebuttons(true);
		           	}else{
		           		$('input[name=txtDocNo]').val(val.DocNum).prop('disabled',true);
		            	$('input[name=txtVendor]').val(val.CardCode).prop('disabled',false);
	...
	$('#PRModal').on('shown.bs.modal',function(){
		...
		$('#PRCont').load('../../proc/views/PO/vw_prdoclist.php?servicetype=' + encodeURI(servicetype),function(){
	...
	$(document.body).on('click','#btnChoose',function(){
        ...
        setTimeout(function(){
        	$('#modal-load-init').modal('hide');
        	$('#modal-load-init').modal('show');
        	populatedetPRMulti(docentry,CompanyDb,function(){
    			//$('input[name=TotBefDisc]').trigger('keyup');
    			isSinglePR = false;
	...
	//Select Document Table Row Click
	$(document.body).on('dblclick','#tblPRDocument tbody > tr',function(){
	...
	// 	if(docentry != '')
	// 	{
	// 		window.open("../../report/PO/po-report.php?docentry=" + encodeURI(docentry), "", "width=1130,height=550,left=220,top=110");
	// 	}
	// })
	//End Print Document
  ...
  $(document.body).on('click','#btnSelectPrint > li > a', function () 
    ...
    var ServerIP = $('input[name=SERVER_IP]').val(); 
    var port = $('input[name=CR_PORT]').val(); 
    var path = $('input[name=CR_PATH]').val(); 

    if(docentry != '')
    {
      if(host == 'localhost' || host == ServerName || host == ServerIP)
				{
					if(href == '#PO1')
					{
						window.open("http://" + encodeURI(ServerIP) + ":" + encodeURI(port) + encodeURI(path) + "Nabaja_Purchase_Order" + "/" + encodeURI(docentry),"","width=1130,height=550,left=220,top=110");
					}
					else
					{
						window.open("http://" + encodeURI(ServerIP) + ":" + encodeURI(port) + encodeURI(path) + "Error" + "/" + encodeURI(docentry),"","width=1130,height=550,left=220,top=110");
					}
				}
				else
				{
					if(href == '#PO1')
					{
						window.open("http://" + encodeURI(host) + ":" + encodeURI(port) + encodeURI(path) + "Nabaja_Purchase_Order" + "/" + encodeURI(docentry),"","width=1130,height=550,left=220,top=110");
					}
					else
					{
						window.open("http://" + encodeURI(host) + ":" + encodeURI(port) + encodeURI(path) + "Error" + "/" + encodeURI(docentry),"","width=1130,height=550,left=220,top=110");
					}
  ...
  //End Print Document
	...
	$('#SerialModal').on('shown.bs.modal',function(){
		...
		if(lineno == '')
		{
			$.ajax({
			type: "POST",
			url: "../../proc/views/PO/vw_itemcode-serial.php",
		...
		else
		{
			...
			$.ajax({
			type: "POST",
			url: "../../proc/views/PO/vw_setserial.php",
			data: form_data,
			success: function(data)
				{
	...
	//SELECT Serial
		...
		$(document.body).on('click','#btnPOListVIEW',function(e)
	{
		$('#resView').empty();
	...
	$('#POListModal').on('shown.bs.modal',function()
	{
		$('#resView').append('<p class="text-center" id="itm-loader"><img src=../../img/ajax-loader.gif /> Loading...</p>');
	...
	$('#POListModal').on('hide.bs.modal',function()
	{
		$('#resView').empty();
	});
	...
	$(document.body).on('click','#btnPOListPDF',function(e)
	{
    	var err = 0;
    	var errmsg = '';
    	var txtDateFrom = $('input[name=txtDateFrom]').val();
		...
		if(err == 0)
		{
			window.open("../../report/po/polist-report.php?txtAPVListFrom=" + encodeURI(txtAPVListFrom) + "&txtAPVListTo=" + encodeURI(txtAPVListTo) + "&txtDateFrom=" + encodeURI(txtDateFrom) + "&txtDateTo=" + encodeURI(txtDateTo), "", "width=1130,height=550,left=220,top=110");
		}
		else
		{
	...
	$(document.body).on('click','#btnPOListEXCEL',function(e)
	{
    	var err = 0;
    	var errmsg = '';
    	var txtDateFrom = $('input[name=txtDateFrom]').val();
		...
		if(err == 0)
		{
			window.open("../../report/po/polist-excel.php?txtAPVListFrom=" + encodeURI(txtAPVListFrom) + "&txtAPVListTo=" + encodeURI(txtAPVListTo) + "&txtDateFrom=" + encodeURI(txtDateFrom) + "&txtDateTo=" + encodeURI(txtDateTo), "", "width=1130,height=550,left=220,top=110");
		}
		else
		{
			notie.alert(3, errmsg, 2);
	...
	//Save PO
    $(document.body).on('click','#btnSavePO',function(e){
		...
		var txtPRRef = $('input[name=txtPRRef]').val();
		...
		//var appstatus = $('input[name=txtAppStatus]').val();
		...
		var tbl2 = $('#tblDetails tbody tr').each(function(i) {  
		        ...
		        if(servicetype == 'I'){
		        	...
		        	if($(this).find('td.ftext').text() == 'N'){
						...
						itArr.push('"' + $(this).find('input.itemname').val().replace(/"/g, '\'') + '"');
		...
		if(isSinglePR){
			urlstr = '../../proc/exec/PO/exec-savepo.php';
		}else{
			urlstr = '../../proc/exec/PO/exec-savepo-multi.php';
		}
    		...
    		$.ajax({
                ...
                success: function(html){
					...
					if(res[0] == 'true'){
						...
						disablebuttons(true)
						setTimeout(function(){
							location.replace('../../forms/PO/PO.php');
						},2000)
    ...
    $(document.body).on('click','#btnUpdate',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/PO/exec-updatepo.php',
                data: {
                		json : json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
                		basentry : basentry,
                		vendor : vendor,
	...
	//Canceled SO
    $(document.body).on('click','#btnCancelPO',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/PO/exec-cancelpo.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
	...
	//Closed SO
    $(document.body).on('click','#btnClosePO',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/PO/exec-closepo.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
	...
	function checkitemcode(itemcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/PO/vw_checkitemcode.php',
	        async: false,
	        data: { 
					itemcode : itemcode
			},
	...
	function checkwhs(whscode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/PO/vw_checkwhscode.php',
	        async: false,
	        data: {
				whscode : whscode
			}, 
	...
	function checkacctcode(acctcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/PO/vw_checkacctcode.php',
	        async: false,
	        data: {
				acctcode : acctcode
			},
	...
	function checkbpcode(bpcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/PO/vw_checkbpcode.php',
	        async: false,
	        data: {
				bpcode : bpcode
			},
	...
	function populatedet(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/PO/vw_documentdetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetPO(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/PO/vw_podetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetPR(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/PO/vw_prdetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetPRMulti(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/PO/vw_prdetailsdata-multi.php?docentry=' + encodeURI(docentry),function(result){

...
{
	$('#window-title').text('Projects Setup');
	$('#mod-title').text('Projects Setup');
	...
	$('#DocumentModal').on('shown.bs.modal',function()
	{
		...
		$('#DocumentCont').load('../../proc/views/PS/vw_doclist.php?servicetype=' + encodeURI(servicetype),function()
		{
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight)
				{
		            if($(this).scrollTop() > 0)
					{
		                ...
		                if(ctr == 0)
						{
							...
							$.ajax({
								type: 'POST',
		                        url: '../../proc/views/PS/vw_doclist-load.php',
		                        data: {
									itemcode : itemcode
								},
	...
	$(document.body).on('keyup','input[name=DocumentSearch]',function()
	{
        ...
        $('#DocumentCont table tbody').load('../../proc/views/PS/vw_doclist-load.php?srchval=' + encodeURI(searchVal));
	});
	//End Search Document
    ...
    $(document.body).on('click','#btnSavePS',function(e)
	{
		var err = 0;
    	var errmsg = '';
		...
		if(err == 0)
		{
			...
			$.ajax({
                type: 'POST',
                url: '../../proc/exec/PS/exec-save.php',
                data: {
                ...
                success: function(html)
				{
					...
					if(res[0] == 'true')
					{
						...
						disablebuttons(true)
						setTimeout(function(){
							location.replace('../../forms/PS/PS.php');
						},2000);
					}
					else
					{
	...
	$(document.body).on('click','#btnUpdate',function(e)
	{
    	...
    	if(err == 0)
		{
			...
			$.ajax({
                type: 'POST',
                url: '../../proc/exec/PS/exec-update.php',
                data: {
                		txtPrcCode : txtPrcCode,
                		txtPrcName : txtPrcName,
                		ChkActive : ChkActive
	...
	//Canceled SO
    $(document.body).on('click','#btnCancelPS',function(e)
	{
    	var err = 0;
    	var errmsg = '';
    	var txtPrcCode = $('input[name=txtPrcCode]').val();
    	...
    	if(err == 0)
		{
			...
			$.ajax({
                type: 'POST',
                url: '../../proc/exec/PS/exec-remove.php',
                data: {
                		txtPrcCode : txtPrcCode,
                		txtPrcName : txtPrcName,
                		ChkActive : ChkActive

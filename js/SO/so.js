...
function view_po(docentry) 
{
	var docentry = docentry;
	
	if(docentry != '' || docentry != null || docentry != 0)
	{
		var url = "SO.php?DocEntry="+ docentry;
...
function getDocCur(selDocCur)
	{
		...
		if(x == 'C') {
			...
			$('select[name=selDocCur]').load('../../proc/views/SO/vw_doccur.php', function () {
...
$(document).ready(function() {
	...
	//Initialize Title
	$('#mod-title').text('Sales Order');
	//End Initialize Title

	
    //Load SO Series 
    $('#btnSeries').html('Loading...')
    $('#SeriesList').load('../../proc/views/SO/vw_series.php?objtype=17',function(){
	...
	$('select[name=txtSalesEmployee]').load('../../proc/views/SO/vw_salesemployee.php');
	
	$('select[name=txtPayment]').html('<option>Loading...</option>');
	$('select[name=txtPayment]').load('../../proc/views/SO/vw_paymentterms.php');

	$('#OwnerModal').on('shown.bs.modal', function () {

        $('#OwnerCont').html('<p class="text-center"><img src=../../img/ajax-loader.gif /> Loading...</p>');
        $('#OwnerCont').load('../../proc/views/SO/vw_employee.php?CardType=C', function () {
            //Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SO/vw_employee-load.php',
    ...
    $(document.body).on('keyup', 'input[name=OwnerSearch]', function () {
        ...
        $('#OwnerCont table tbody').load('../../proc/views/SO/vw_employee-load.php?srchval=' + encodeURI(searchVal));
    })
    //End Search Owner
	...
	$(document.body).on('change','select[name=cmbServiceType]',function()
	{
		...
		if(servicetype == 'I')
		{
			...
			$('#ModDetails').load('../../forms/SO/SO-details.php?servicetype=I',function()
			{
		...
		else if(servicetype == 'S')
		{
			...
			$('#ModDetails').load('../../forms/SO/SO-details.php?servicetype=S',function()
			{
				//Clear value Total Before Discount
				$('input[name=TotBefDisc]').val('');
				//End Clear value Total Before Discount
	...
	$(document.body).on('click','#btnAddRow',function()
	{
		...
		if(servicetype == 'I'){
			//Item Type
			$(this).load('../../forms/SO/SO-details-row.php?servicetype=I',function(result){
		...
		{
			//Service Type
			$(this).load('../../forms/SO/SO-details-row.php?servicetype=S',function(result){
				$('#tblDetails tbody').append(result);
	...
	$(document.body).on('click','#btnDelRow',function()
	{
		...
		else
		{
			...
			$.ajax({
				type: 'POST',
				url: '../../proc/exec/so/exec-delete-so.php',
				data: 
				{
	...
	$('#ItemModal').on('shown.bs.modal',function()
	{
		...
		$('#ItemCont').load('../../proc/views/SO/vw_itemlist.php',function()
		{
		        ...
		        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) 
				{
                    if ($(this).scrollTop() > 0) 
					{
                        ...
                        if (ctr == 0) 
						{
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SO/vw_itemlist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) 
	...
	$(document.body).on('dblclick','#tblItem tbody > tr',function()
	{
		...
		$.ajax({
		type: "POST",
		url: "../../proc/views/SO/vw_pricelist.php",
		data: form_data,
		success: function(html)
			{
				result = html;
	...
	$(document.body).on('keyup','input[name=ItemSearch]',function()
	{
        ...
        $('#ItemCont table tbody').load('../../proc/views/SO/vw_itemlist-load.php?srchval=' + encodeURI(searchVal));
	});
	...
	$(document.body).on('blur','#tblDetails .itemcode',function()
	{
		...
		if(item[7] == 'T')
		{
			var appendbom = '';
			//Set Qty of Father
			$('.selected-det').find('input.qty').val(item[8] / item[8]);
			
			//End Set Qty of Father

			//Check if bom is already appended
			appendbom = $('.selected-det').attr('bom-val');
			//End Check if bom is already appended
			...
			if(appendbom != 1){
				...
				//Get BOM
				$('#btnAddRow').load('../../proc/views/SO/vw_bom.php?itemcode=' + encodeURI(item[0]),function(result){
					$('.selected-det').after(result);
					...
					$('input[name=TotBefDisc]').trigger('keyup');
					//End Compute Total Before Discount

					//Delete First Row with Bom
					$('#btnDelRow').trigger('click');
					//End Delete First Row with Bom

					$('.selected-det').attr('bom-val','1');
	...
	$('#WhsModal').on('shown.bs.modal',function()
	{
		...
		$('#WhsCont').load('../../proc/views/SO/vw_whslist.php',function()
		{
	...
	$(document.body).on('keyup','input[name=WhsSearch]',function(){
        ...
        $('#WhsCont table tbody').load('../../proc/views/SO/vw_whslist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Whs
	...
	$('#DepartmentModal').on('shown.bs.modal',function()
	{
		...
		$('#DepartmentCont').load('../../proc/views/PO/vw_deptlist.php');
	...
	$(document.body).on('keyup','input[name=DepartmentSearch]',function(){
        ...
        $('#DepartmentCont table tbody').load('../../proc/views/PO/vw_deptlist-load.php?srchval=' + encodeURI(searchVal));
	})
	...
	$('#ProjectModal').on('shown.bs.modal',function()
	{
		...
		$('#ProjectCont').load('../../proc/views/PO/vw_projlist.php');
	...
	$(document.body).on('keyup','input[name=ProjectSearch]',function(){
        ...
        $('#ProjectCont table tbody').load('../../proc/views/PO/vw_projlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Department
	...
	$('#EmployeeModal').on('shown.bs.modal',function()
	{
		...
		$('#EmployeeCont').load('../../proc/views/PO/vw_emplist.php');
	...
	$(document.body).on('keyup','input[name=EmployeeSearch]',function(){
        ...
        $('#EmployeeCont table tbody').load('../../proc/views/PO/vw_equiplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Employee
	...
	$('#EquipmentModal').on('shown.bs.modal',function()
	{
		...
		$('#EquipmentCont').load('../../proc/views/PO/vw_equiplist.php');
	...
	$(document.body).on('keyup','input[name=EquipmentSearch]',function(){
		...
		$('#EquipmentCont table tbody').load('../../proc/views/PO/vw_equiplist-load.php?srchval=' + encodeURI(searchVal));
	})
	...
	$('#AcctModal').on('shown.bs.modal',function(){
		...
		$('#AcctCont').load('../../proc/views/SO/vw_acctlist.php',function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/SO/vw_acctlist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('keyup','input[name=AcctSearch]',function(){
        ...
        $('#AcctCont table tbody').load('../../proc/views/SO/vw_acctlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Acct
	...
	$('#InvDataModal').on('shown.bs.modal',function(){
		...
		if(itemcode != ''){
			...
			$('#InvDataCont').load('../../proc/views/SO/vw_invdatalist.php?itemcode=' + itemcode,function(){
	...
	$('#DocumentModal').on('shown.bs.modal',function()
	{
		...
		$('#DocumentCont').load('../../proc/views/SO/vw_doclist.php?servicetype=' + encodeURI(servicetype),function(){
			//Add Scroll Function 
				...
				if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SO/vw_doclist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$(document.body).on('keyup','input[name=DocumentSearch]',function(){
        ...
        $('#DocumentCont table tbody').html('<tr><td class="text-center" colspan="8"><img src=../../img/ajax-loader.gif /> Loading...</td></tr>');
        $('#DocumentCont table tbody').load('../../proc/views/SO/vw_doclist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Document
	...
	$(document.body).on('keyup','input[name=txtDocEntry]',function(){
		...
		$.getJSON('../../proc/views/SO/vw_getdocumentdata.php?docentry=' + docentry, function(data) {
            /* data will hold the php array as a javascript object */
	            ...
	            $.each(data, function(key, val) {
                  ...
                  $('select[name=txtU_SalesCoordinator]').val(val.U_SalesCoordinator).prop('disabled', true);
		            	...
		            	if(val.ContactName != '') {
							$('select[name=selContactPerson]').html('<option value="'+val.CntctCode+'" selected>'+val.Name+'</option>').prop('disabled',true);
						}
						else{
							//Load contact list
							$('select[name=selContactPerson]').html('Loading...');
							$('select[name=selContactPerson]').load('../../proc/views/SO/vw_contactlist.php?cardcode=' + encodeURI(val.CardCode));
						}
						...
						$('select[name=selShipTo]').load('../../proc/views/SO/vw_shipto.php?cardcode=' + val.CardCode, function () {
						...
						$('select[name=selBillTo]').load('../../proc/views/SO/vw_billto.php?cardcode=' + val.CardCode, function () 
						{
							$('select[name=selBillTo]').val(val.PayToCode).prop('disabled',true);
						});
						
						$('input[name=txtListNum]').val(val.ListNum).prop('disabled',true);
						$('input[name=txtAppStatus]').val(val.PoStatus).prop('disabled',true);
                  ...
                  $('select[name=txtU_SalesCoordinator]').val(val.U_SalesCoordinator).prop('disabled', false);
						...
						$('input[name=txtCustomerName]').val(val.U_CustomerName).prop('disabled',false);
		            	if(val.ContactName != '') {
							$('select[name=selContactPerson]').html('<option value="'+val.CntctCode+'" selected>'+val.Name+'</option>').prop('disabled',false);
						}
						else{
							//Load contact list
							$('select[name=selContactPerson]').html('Loading...');
						...
						$('select[name=selShipTo]').html('<option>Loading...</option>');
						$('select[name=selShipTo]').load('../../proc/views/SO/vw_shipto.php?cardcode=' + val.CardCode, function () {
							$('select[name=selShipTo]').val(val.ShipToCode).prop('disabled',false);
							})
						...
						$('select[name=selBillTo]').html('<option>Loading...</option>');
						$('select[name=selBillTo]').load('../../proc/views/SO/vw_billto.php?cardcode=' + val.CardCode, function () {
							$('select[name=selBillTo]').val(val.PayToCode).prop('disabled',false);
							})
	...
	$('#BPModal').on('shown.bs.modal',function()
	{
		...
		$('#BPCont').load('../../proc/views/SO/vw_bplist.php?CardType=C',function()
		{
			//Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SO/vw_bplist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$(document.body).on('dblclick','#tblBP tbody > tr',function(){
		...
		//Load contact list
		$('select[name=selContactPerson]').html('Loading...');
		$('select[name=selContactPerson]').load('../../proc/views/SO/vw_contactlist.php?cardcode=' + encodeURI(BPCode));
		...
		//Load Ship To
		$('select[name=selShipTo]').html('<option>Loading...</option>');
		$('select[name=selShipTo]').load('../../proc/views/SO/vw_shipto.php?cardcode=' + encodeURI(BPCode));
		
		//Load Bill To
		$('select[name=selBillTo]').html('<option>Loading...</option>');
		$('select[name=selBillTo]').load('../../proc/views/SO/vw_billto.php?cardcode=' + encodeURI(BPCode));
	...
	$(document.body).on('keyup','input[name=BPSearch]',function(){
        ...
        $('#BPCont table tbody').load('../../proc/views/SO/vw_bplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search BP
	...
	$(document.body).on('keyup','input[name=txtBaseEntry]',function(){
		...
		if(basentry != ''){
			...
			$.getJSON('../../proc/views/SO/vw_getsqdata.php?docentry=' + basentry, function(data) {
	            /* data will hold the php array as a javascript object */
           		...
           		$.each(data, function(key, val) {
	            	...
	            	if(val.DocStatus == 'C'){
		            	...
		            	$('textarea[name=txtRemarksF]').val(val.Comments + ' Based on Sales Quotation ' + val.DocNum + '.').prop('disabled',true);
						...
						$('textarea[name=txtRemarksF]').val(val.Comments + ' Based on Sales Quotation ' + val.DocNum + '.').prop('disabled',false);
	            ...
	            setTimeout(function(){
	            	$('#modal-load-init').modal('hide');
	            	$('#modal-load-init').modal('show');
	            	populatedetSQ(basentry,function(){
            			//$('input[name=TotBefDisc]').trigger('keyup')
		            	$('#modal-load-init').modal('hide');
	...
	$(document.body).on('click','#btnPrint',function(){
		...
		if(docentry != ''){
			...
			window.open("../../report/SO/so-report.php?docentry=" + encodeURI(docentry), "", "width=1130,height=550,left=220,top=110");
	...
	$('#EmailModal').on('shown.bs.modal',function(){
		...
		$.ajax({
		type: "POST",
		url: "../../proc/views/SO/vw_email.php",
	...
	$('#SerialModal').on('shown.bs.modal',function(){
		...
		if(lineno == '')
		{
			$.ajax({
			type: "POST",
			url: "../../proc/views/SO/vw_itemcode-serial.php",
		...
		else
		{
			...
			$.ajax({
			type: "POST",
			url: "../../proc/views/SO/vw_setserial.php",
			data: form_data,
			success: function(data)
				{
	...
	//Save PO
    $(document.body).on('click','#btnSaveSO',function(e){
		...
		var shipto = $('select[name=selShipTo]').val();
		var billto = $('select[name=selBillTo]').val();
		...
		if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SO/exec-saveso.php',
                data: {
                		json : json.replace(/(\r\n|\n|\r)/gm, ''),
                		basentry : basentry,
                		vendor : vendor,
	...
	$(document.body).on('click','#btnAddContact',function(e){
		...
		if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SO/exec-savecontact.php',
                data: {
    ...
    $(document.body).on('click','#btnUpdate',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SO/exec-updateso.php',
                data: {
                		json : json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
                		basentry : basentry,
                		vendor : vendor,
	...
	//Canceled SO
    $(document.body).on('click','#btnCancelSO',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SO/exec-cancelso.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
	...
	//Closed SO
    $(document.body).on('click','#btnCloseSO',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SO/exec-closeso.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
  ...
  // End Sales Coordinator
	...
	function checkitemcode(itemcode)
	{
		...
		$.ajax({
	        type: 'POST',
	        url: '../../proc/views/SO/vw_checkitemcode.php',
	        async: false,
	        data: { 
					itemcode : itemcode
			},
	...
	function checkwhs(whscode)
	{
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SO/vw_checkwhscode.php',
	        async: false,
	        data: {
				whscode : whscode
			}, 
	...
	function checkacctcode(acctcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SO/vw_checkacctcode.php',
	        async: false,
	        data:  {
				acctcode : acctcode
			},
	...
	function populatedet(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/SO/vw_documentdetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetSQ(docentry,CompanyDb,callback){

			
		$('#tblDetails tbody').load('../../proc/views/SO/vw_sqdetailsdata.php?docentry=' + docentry,function(result){

...
function getDocCur(selDocCur)
	{
		...
		if(x == 'C') {
			...
			$('select[name=selDocCur]').load('../../proc/views/APDP/vw_doccur.php', function () {
...
{
	$('#window-title').text('A/P Down Payment');
	...
	//Initialize Title
	$('#mod-title').text('A/P Down Payment');
	//End Initialize Title

	
    //Load Grpo Series 
    $('#btnSeries').html('Loading...')
    $('#SeriesList').load('../../proc/views/APDP/vw_series.php?objtype=204',function(){
	...
	$('select[name=txtSalesEmployee]').load('../../proc/views/APDP/vw_salesemployee.php');
	
	$('select[name=txtPayment]').html('<option>Loading...</option>');
	$('select[name=txtPayment]').load('../../proc/views/APDP/vw_paymentterms.php');
    ...
    $('#OwnerModal').on('shown.bs.modal', function () {
        ...
        $('#OwnerCont').load('../../proc/views/APDP/vw_employee.php?CardType=C', function () {
            //Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/APDP/vw_employee-load.php',
    ...
    $(document.body).on('keyup', 'input[name=OwnerSearch]', function () {
        ...
        $('#OwnerCont table tbody').load('../../proc/views/APDP/vw_employee-load.php?srchval=' + encodeURI(searchVal));
    })
    //End Search Owner
    ...
    $(document.body).on('click','.series',function(){
 	...
 	$(document.body).on('change','select[name=cmbServiceType]',function(){
		...
		if(servicetype == 'I'){
			...
			$('#ModDetails').load('../../forms/APDP/APDP-details.php?servicetype=I',function(){
			...
			$('#ModDetails').load('../../forms/APDP/APDP-details.php?servicetype=S',function(){
				//Clear value Total Before Discount
				$('input[name=TotBefDisc]').val('');
				//End Clear value Total Before Discount
				cback = 0;
	...
	$(document.body).on('click','#btnAddRow',function(){
		...
		if(servicetype == 'I'){
			//Item Type
			$(this).load('../../forms/APDP/APDP-details-row.php?servicetype=I',function(result){
			...
			//Service Type
			$(this).load('../../forms/APDP/APDP-details-row.php?servicetype=S',function(result){
				$('#tblDetails tbody').append(result);
	...
	$(document.body).on('click','#btnFreeText',function(){
		...
		if(servicetype == 'I'){
			//Item Type
			$(this).load('../../forms/APDP/APDP-details-row.php?servicetype=I&freetext=1',function(result){
	...
	$('#ItemModal').on('shown.bs.modal',function(){
		...
		$('#ItemCont').load('../../proc/views/APDP/vw_itemlist.php',function(){
			//Add Scroll Function 
		        ...
		        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/APDP/vw_itemlist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$(document.body).on('dblclick','#tblItem tbody > tr',function(){
		...
		var form_data =
			...
			url: "../../proc/views/APDP/vw_pricelist.php",
			data: form_data,
			success: function(html)
				{
					result = html;
		...
		$('.selected-det').find('input.warehouse').val('W01-000');
		//End Details Item
	...
	$(document.body).on('keyup','input[name=ItemSearch]',function(){
        ...
        $('#ItemCont table tbody').load('../../proc/views/APDP/vw_itemlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Item
	...
	$('#WTaxCodeModal').on('shown.bs.modal',function()
	{
		...
		$('#WTaxCodeCont').load('../../proc/views/APDP/vw_withholdingtaxtable.php?vendorcode=' + encodeURI(vendor) ,function()
		{
	...
	$(document.body).on('dblclick', '#tblWTaxCode tbody > tr', function () 
	{
		...
		$('#tblWTAXCodeList tbody').append('<tr><td style="padding-top: 2px;  padding-bottom: 2px;" class="rowno text-center">' + rowno + '</td>'+
																	...
																	'<td style="padding-top: 2px;  padding-bottom: 2px;"><input type="type" class="taxableamount" value="' +parseFloat(linetotal).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') +'"></td>'+
		...
		var WTAXAmount = parseFloat(linetotal) * parseFloat(rate/100);
	...
	$('#WhsModal').on('shown.bs.modal',function(){
			...
			$('#WhsCont').load('../../proc/views/APDP/vw_whslist.php',function(){
	...
	$(document.body).on('keyup','input[name=WhsSearch]',function(){
        ...
        $('#WhsCont table tbody').load('../../proc/views/APDP/vw_whslist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Whs
	...
	$('#DepartmentModal').on('shown.bs.modal',function()
	{
		...
		$('#DepartmentCont').load('../../proc/views/APDP/vw_deptlist.php');
	...
	$(document.body).on('keyup','input[name=DepartmentSearch]',function(){
        ...
        $('#DepartmentCont table tbody').load('../../proc/views/APDP/vw_deptlist-load.php?srchval=' + encodeURI(searchVal));
	})
	...
	$('#ProjectModal').on('shown.bs.modal',function()
	{
		...
		$('#ProjectCont').load('../../proc/views/APDP/vw_projlist.php');
	...
	$(document.body).on('keyup','input[name=ProjectSearch]',function(){
        ...
        $('#ProjectCont table tbody').load('../../proc/views/APDP/vw_projlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Department
	...
	$('#EmployeeModal').on('shown.bs.modal',function()
	{
		...
		$('#EmployeeCont').load('../../proc/views/APDP/vw_emplist.php');
	...
	$(document.body).on('keyup','input[name=EmployeeSearch]',function(){
        ...
        $('#EmployeeCont table tbody').load('../../proc/views/APDP/vw_emplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Employee
		...
		$('#EquipmentModal').on('shown.bs.modal',function()
		{
			...
			$('#EquipmentCont').load('../../proc/views/APDP/vw_equiplist.php');
		...
		$(document.body).on('keyup','input[name=EquipmentSearch]',function(){
			...
			$('#EquipmentCont table tbody').load('../../proc/views/APDP/vw_equiplist-load.php?srchval=' + encodeURI(searchVal));
		})
	...
	$('#AcctModal').on('shown.bs.modal',function(){
		...
		$('#AcctCont').load('../../proc/views/APDP/vw_acctlist.php',function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/APDP/vw_acctlist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('keyup','input[name=AcctSearch]',function(){
        ...
        $('#AcctCont table tbody').load('../../proc/views/APDP/vw_acctlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Acct
	...
	$('#InvDataModal').on('shown.bs.modal',function(){
		...
		if(itemcode != ''){
			...
			$('#InvDataCont').load('../../proc/views/APDP/vw_invdatalist.php?itemcode=' + itemcode,function(){
	...
	$('#DocumentModal').on('shown.bs.modal',function(){
		...
		$('#DocumentCont').load('../../proc/views/APDP/vw_doclist.php?servicetype=' + encodeURI(servicetype),function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/APDP/vw_doclist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('keyup','input[name=DocumentSearch]',function(){
        ...
        $('#DocumentCont table tbody').load('../../proc/views/APDP/vw_doclist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Document
	...
	$(document.body).on('keyup','input[name=txtDocEntry]',function(){
		...
		$.getJSON('../../proc/views/APDP/vw_getdocumentdata.php?docentry=' + docentry, function(data) {
	...
	$('#BPModal').on('shown.bs.modal',function(){
		...
		$('#BPCont').load('../../proc/views/APDP/vw_bplist.php?CardType=S',function(){
			//Add Scroll Function 
		        ...
		        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/APDP/vw_bplist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$(document.body).on('dblclick','#tblBP tbody > tr',function(){
		...
		$('textarea[name=txtShipTo]').load('../../proc/views/APDP/vw_shipto.php?cardcode=' + encodeURI(BPCode));
		
		//Load Bill To
		$('textarea[name=txtBillTo]').html('Loading...');
		$('textarea[name=txtBillTo]').load('../../proc/views/APDP/vw_billto.php?cardcode=' + encodeURI(BPCode));
	...
	$(document.body).on('keyup','input[name=BPSearch]',function(){
        ...
        $('#BPCont table tbody').load('../../proc/views/APDP/vw_bplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search BP
	...
	$(document.body).on('keyup','input[name=txtBaseEntry]',function(){
		...
		if(basentry != ''){
			//Show Base Document
			//Get PR data using JSON
			$.getJSON('../../proc/views/APDP/vw_getpodata.php?docentry=' + basentry, function(data) {
	            /* data will hold the php array as a javascript object */
           		...
           		$.each(data, function(key, val) {
	            	...
	            	if(val.DocStatus == 'C'){
		            	...
		            	$('textarea[name=txtRemarksF]').val(val.Comments + 'PO#' + val.NumAtCard + '.').prop('disabled',true);
						...
						$('textarea[name=txtRemarksF]').val(val.Comments + 'PO#' + val.NumAtCard + '.').prop('disabled',false);
						$('textarea[name=txtShipTo]').val(val.Address2).prop('disabled',false);
		            	$('textarea[name=txtBillTo]').val(val.Address).prop('disabled',false);
		            	setTimeout(function(){
			            	$('input[name=TotBefDisc]').val(val.TotBefDisc);
	...
	$(document.body).on('click','#btnPrint',function(){
		...
		if(docentry != '')
		{
			if(servicetype == 'I')
			{
				window.open("../../report/APDP/apdp-report.php?docentry=" + encodeURI(docentry), "", "width=1130,height=550,left=220,top=110");
			}
			else
			{
				window.open("../../report/APDP/apdp-report-s.php?docentry=" + encodeURI(docentry), "", "width=1130,height=550,left=220,top=110");
			}
		}
	})
	//End Print Document
    ...
    $('#POModal').on('shown.bs.modal', function () {
        ...
        $('#POCont').load('../../proc/views/APDP/vw_podoclist.php?vendor=' + encodeURI(vendor), function () {
            //Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/APDP/vw_podoclist-load.php?vendor=' + encodeURI(vendor),
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
    ...
    $(document.body).on('keyup', 'input[name=POSearch]', function () {
        ...
        $('#POCont table tbody').load('../../proc/views/APDP/vw_podoclist-load.php?srchval=' + encodeURI(searchVal) + '&vendor=' + encodeURI(vendor));
    })
    //End Search Document
	...
	$(document.body).on('click','#btnAPDPListVIEW',function(e)
	{
		$('#resView').empty();
    	...
    	var txtAPDPListFrom = $('input[name=txtAPDPListFrom]').val();
    	var txtAPDPListTo = $('input[name=txtAPDPListTo]').val();
		
		if(err == 0)
		{
			$.ajax({
				type: 'POST',
				url: '../../report/apdp/rpt_view.php',
				data: {
					txtDateFrom : txtDateFrom,
					txtDateTo : txtDateTo,
					txtAPDPListFrom : txtAPDPListFrom,
					txtAPDPListTo : txtAPDPListTo
				},
				success: function (html) 
				{
					$('#resView').html(html);
	...
	$('#APDPListModal').on('shown.bs.modal',function()
	{
		$('#resView').append('<p class="text-center" id="itm-loader"><img src=../../img/ajax-loader.gif /> Loading...</p>');
	...
	$('#APDPListModal').on('hide.bs.modal',function()
	{
		$('#resView').empty();
	});
	...
	$(document.body).on('click','#btnAPDPListPDF',function(e)
	{
    	var err = 0;
    	var errmsg = '';
		var txtDateFrom = $('input[name=txtDateFrom]').val();
    	...
    	var txtAPDPListTo = $('input[name=txtAPDPListTo]').val();
			...
			$('.apdplistrequired').each(function()
			{
    			if($(this).val() == '')
				{
		...
		if(err == 0)
		{
			window.open("../../report/apdp/apdplist-report.php?txtAPDPListFrom=" + encodeURI(txtAPDPListFrom) + "&txtAPDPListTo=" + encodeURI(txtAPDPListTo) + "&txtDateFrom=" + encodeURI(txtDateFrom) + "&txtDateTo=" + encodeURI(txtDateTo), "", "width=1130,height=550,left=220,top=110");
		}
		else
		{
	...
	$(document.body).on('click','#btnAPDPListEXCEL',function(e)
	{
    	var err = 0;
    	var errmsg = '';
		var txtDateFrom = $('input[name=txtDateFrom]').val();
		...
		if(err == 0)
		{
			window.open("../../report/apdp/apdplist-excel.php?txtAPDPListFrom=" + encodeURI(txtAPDPListFrom) + "&txtAPDPListTo=" + encodeURI(txtAPDPListTo) + "&txtDateFrom=" + encodeURI(txtDateFrom) + "&txtDateTo=" + encodeURI(txtDateTo), "", "width=1130,height=550,left=220,top=110");
		}
		else
		{
			notie.alert(3, errmsg, 2);
    ...
    $(document.body).on('click','#btnSaveAPV',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/APDP/exec-saveapdp.php',
                data: {
                		json : json.replace(/(\r\n|\n|\r)/gm, ''),
                		json1 : json1.replace(/(\r\n|\n|\r)/gm, ''),
                		basentry : basentry,
                ...
                success: function(html){
					...
					if(res[0] == 'true'){
						...
						disablebuttons(true)
						setTimeout(function(){
							location.replace('../../forms/APDP/APDP.php');
						},2000)
    ...
    $(document.body).on('click','#btnCancelAPV',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/APDP/exec-cancelapv.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
    ...
    $(document.body).on('click','#btnCloseAPV',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/APDP/exec-closeAPDP.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
	...
	function checkitemcode(itemcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/APDP/vw_checkitemcode.php',
	        async: false,
	        data: {
				itemcode : itemcode
			},
	...
	function checkwhs(whscode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/APDP/vw_checkwhscode.php',
	        async: false,
	        data: {
				whscode : whscode
			},
	...
	function checkacctcode(acctcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/APDP/vw_checkacctcode.php',
	        async: false,
	        data: {
				acctcode : acctcode
			},
	...
	function checkbpcode(bpcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/APDP/vw_checkbpcode.php',
	        async: false,
	        data: {
				bpcode : bpcode
			},
	...
	function populatedetPOMulti(docentry,callback) {
        ...
        $('#tblDetails tbody').load('../../proc/views/APDP/vw_podetailsdata-multi.php?docentry=' + encodeURI(docentry), function (result) {
	...
	{
		$('#tblWTAXCodeList tbody').load('../../proc/views/APDP/vw_withholdingtaxtablelist.php?docentry=' + docentry,function(result)
		{
			callback();
		});
	}
	...
	function populatedet(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/APDP/vw_documentdetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetPO(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/APDP/vw_podetailsdata.php?docentry=' + docentry,function(result){
			callback();

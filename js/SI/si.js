$(window).load(function() {
    ...
    $('#window-title').text('A/R Invoice');
	//End Trigger BaseEntry
	
	$('select[name=cmbServiceType]').val('S').trigger('change');
...
function getDocCur(selDocCur)
	{
		...
		if(x == 'C') {
			...
			$('select[name=selDocCur]').load('../../proc/views/SI/vw_doccur.php', function () {
...
$(document).ready(function() 
{
	...
	//Initialize Title
	$('#mod-title').text('A/R Invoice');
	//End Initialize Title

	
    //Load SO Series 
    $('#btnSeries').html('Loading...');
    $('#SeriesList').load('../../proc/views/SI/vw_series.php?objtype=13',function(){
	...
	$('select[name=txtSalesEmployee]').load('../../proc/views/SI/vw_salesemployee.php');
	
	$('select[name=txtPayment]').html('<option>Loading...</option>');
	$('select[name=txtPayment]').load('../../proc/views/SI/vw_paymentterms.php');
   
    $('#OwnerModal').on('shown.bs.modal', function () 
	{
		
        $('#OwnerCont').html('<p class="text-center"><img src=../../img/ajax-loader.gif /> Loading...</p>');
        $('#OwnerCont').load('../../proc/views/SI/vw_employee.php?CardType=C', function () {
            //Add Scroll Function 
                ...
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SI/vw_employee-load.php',
                                data: {
									itemcode : itemcode
								},
    ...
    $(document.body).on('keyup', 'input[name=OwnerSearch]', function () {
        ...
        $('#OwnerCont table tbody').load('../../proc/views/SI/vw_employee-load.php?srchval=' + encodeURI(searchVal));
    })
    //End Search Owner
	...
	$(document.body).on('change','select[name=cmbServiceType]',function()
	{
		...
		if(servicetype == 'I'){
			...
			$('#ModDetails').load('../../forms/SI/SI-details.php?servicetype=I',function(){
			...
			$('#ModDetails').load('../../forms/SI/SI-details.php?servicetype=S',function(){
				//Clear value Total Before Discount
				$('input[name=TotBefDisc]').val('');
				//End Clear value Total Before Discount
				cback = 0;
	...
	$(document.body).on('click','#btnAddRow',function(){
		...
		if(servicetype == 'I'){
			//Item Type
			$(this).load('../../forms/SI/SI-details-row.php?servicetype=I',function(result){
				...
				//$('#tblDetails tbody tr:last').find('input.serialnumber').focus();
			...
			//Service Type
			$(this).load('../../forms/SI/SI-details-row.php?servicetype=S',function(result){
				$('#tblDetails tbody').append(result);
	...
	$('#ItemModal').on('shown.bs.modal',function(){
		...
		$('#ItemCont').load('../../proc/views/SI/vw_itemlist.php',function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/SI/vw_itemlist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('dblclick','#tblItem tbody > tr',function(){
		...
		var form_data =
			...
			$.ajax({
			type: "POST",
			url: "../../proc/views/SI/vw_pricelist.php",
			data: form_data,
			success: function(html)
				{
					result = html;
		...
		$('.selected-det').find('input.warehouse').val(whs);
	...
	$(document.body).on('keyup','input[name=ItemSearch]',function(){
        ...
        $('#ItemCont table tbody').load('../../proc/views/SI/vw_itemlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Item
	...
	$(document.body).on('blur','#tblDetails .itemcode',function()
	{
		...
		if($.trim(item[0]) == '')
		{
			...
			$('.selected-det').find('input.uom').val('');
			$('.selected-det').find('input.price').val('');
		...
		else
		{
			...
			$('.selected-det').find('input.warehouse').val(item[9]);
			
			if(item[3] != 0 || item[3] != '')
			{
				$('.selected-det').find('input.price').val(formatMoney(parseFloat(item[10])));
			}
			else
			{
				$('.selected-det').find('input.price').val('');
	...
	//Item Code Bind
	$(document.body).on('blur','#tblDetails .serialnumber',function(){
		
		var serial = $('.selected-det').find('input.serialnumber').val();
		...
		{
			var item = checkserialno($(this).val()).split(';');
			//item[0] - ItemCode
			//item[1] - FrgnName
			//item[2] - IntrSerial
			//item[3] - SysSerial
			...
			if($.trim(item[0]) == '')
			{
				//notie.alert(1, 'No Serial # Found', 10);
				notie.alert({ type: 3, text: 'No Serial # Found', time: 2 })
			...
			{
				$('.selected-det').find('input.itemcode').val(item[0]);
				$('.selected-det').find('input.itemname').val(item[1]);
				$('.selected-det').find('input.serialno').val(item[3]);
				$('.selected-det').find('input.barcode').val(item[4]);
				$('.selected-det').find('input.grossprice').val(item[5]);
				$('.selected-det').find('input.qty').val(1);
				$('.selected-det').find('input.grossprice').focus();
			}
		}
	...
	$('#WhsModal').on('shown.bs.modal',function(){
			...
			$('#WhsCont').load('../../proc/views/SI/vw_whslist.php',function(){
	...
	$(document.body).on('keyup','input[name=WhsSearch]',function(){
        ...
        $('#WhsCont table tbody').load('../../proc/views/SI/vw_whslist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Whs
	...
	$('#DepartmentModal').on('shown.bs.modal',function()
	{
		...
		$('#DepartmentCont').load('../../proc/views/SI/vw_deptlist.php');
	...
	$(document.body).on('keyup','input[name=DepartmentSearch]',function(){
        ...
        $('#DepartmentCont table tbody').load('../../proc/views/SI/vw_deptlist-load.php?srchval=' + encodeURI(searchVal));
	})
	...
	$('#ProjectModal').on('shown.bs.modal',function()
	{
		...
		$('#ProjectCont').load('../../proc/views/SI/vw_projlist.php');
	...
	$(document.body).on('keyup','input[name=ProjectSearch]',function(){
        ...
        $('#ProjectCont table tbody').load('../../proc/views/SI/vw_projlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Department
	...
	$('#EmployeeModal').on('shown.bs.modal',function()
	{
		...
		$('#EmployeeCont').load('../../proc/views/SI/vw_emplist.php');
	...
	$(document.body).on('keyup','input[name=EmployeeSearch]',function(){
        ...
        $('#EmployeeCont table tbody').load('../../proc/views/SI/vw_emplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Employee
		...
		$('#EquipmentModal').on('shown.bs.modal',function()
		{
			...
			$('#EquipmentCont').load('../../proc/views/SI/vw_equiplist.php');
		...
		$(document.body).on('keyup','input[name=EquipmentSearch]',function(){
			...
			$('#EquipmentCont table tbody').load('../../proc/views/SI/vw_equiplist-load.php?srchval=' + encodeURI(searchVal));
		})
	...
	$(document.body).on('blur','.grossprice',function(e)
	{
		...
		if(grossprice != 0 || grossprice != '')
		{
			$('.selected-det').find('input.grossprice').val(formatMoney(parseFloat(grossprice)));
		}
	});
	...
	$('#AcctModal').on('shown.bs.modal',function(){
		...
		$('#AcctCont').load('../../proc/views/SI/vw_acctlist.php',function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/SI/vw_acctlist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('keyup','input[name=AcctSearch]',function(){
        ...
        $('#AcctCont table tbody').load('../../proc/views/SI/vw_acctlist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Acct
	...
	$('#InvDataModal').on('shown.bs.modal',function(){
		...
		if(itemcode != ''){
			...
			$('#InvDataCont').load('../../proc/views/SI/vw_invdatalist.php?itemcode=' + itemcode,function(){
	...
	$('#DocumentModal').on('shown.bs.modal',function(){
		...
		$('#DocumentCont').load('../../proc/views/SI/vw_doclist.php',function(){
			//Add Scroll Function 
				...
				if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SI/vw_doclist-load.php',
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {
	...
	$(document.body).on('keyup','input[name=DocumentSearch]',function(){
        ...
        $('#DocumentCont table tbody').load('../../proc/views/SI/vw_doclist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search Document
	...
	$(document.body).on('keyup','input[name=txtDocEntry]',function(){
		...
		$.getJSON('../../proc/views/SI/vw_getdocumentdata.php?docentry=' + docentry, function(data) {
			...
			$('#modal-load-init').modal('show');
                  ...
                  // UDFs
                  $('input[name=txtU_ContrctPrice]').val(val.U_ContrctPrice).prop('disabled',true);
                  $('input[name=txtU_Downpaymnt]').val(val.U_Downpaymnt).prop('disabled',true);
                  $('input[name=txtU_MiscFee]').val(val.U_MiscFee).prop('disabled',true);
                  $('input[name=txtU_ResrvationFee]').val(val.U_ResrvationFee).prop('disabled',true);
                  $('select[name=txtU_Realty]').val(val.U_Realty).prop('disabled', true);
                  $('select[name=txtU_SalesCoordinator]').val(val.U_SalesCoordinator).prop('disabled', true);
		            	
		            	
		            	$('input[name=txtCustName]').val(val.CustName).prop('disabled',true);
		            	
						//Load Contact To
						$('select[name=selContactPerson]').html('<option>Loading...</option>');
						$('select[name=selContactPerson]').load('../../proc/views/SI/vw_contactlist.php?cardcode=' + val.CardCode, function () {
						...
						$('select[name=selShipTo]').html('<option>Loading...</option>');
						$('select[name=selShipTo]').load('../../proc/views/SI/vw_sel_shipto.php?cardcode=' + val.CardCode, function () {
							$('select[name=selShipTo]').val(val.ShipToCode).prop('disabled',true);
							})
						
						$('select[name=selBillTo]').html('<option>Loading...</option>');
						$('select[name=selBillTo]').load('../../proc/views/SI/vw_sel_billto.php?cardcode=' + val.CardCode, function () {
							$('select[name=selBillTo]').val(val.PayToCode).prop('disabled',true);
							})
                    ...
                    // with DP
                    $('input[name=txtTaxFWithDp]').val(val.VatSum);
                    $('input[name=txtTotalPaymentDueWithDp]').val(val.DocTotal);
                    $('input[name=txtDpmAppVatBal]').val(val.DpmVat); 
                    $('input[name=txtDpmAmnt]').val(val.DpmAmnt);
		            	...
		            	$('#btnCpy').addClass('hidden');
		            	$('#btnCopyFrom').addClass('hidden');
                  $('#btnDpModal').prop('disabled',true);
						$('#btnCopyFromHide').addClass('hidden');
		           	}else{
		           		$('#btnSeries').html(val.SeriesName);
					    $('#btnSeries').attr('series-val',val.Series);
					    $('#btnSeries').attr('bplid-val',val.BPLId);
                  ...
                  $('select[name=txtU_SalesCoordinator]').val(val.U_SalesCoordinator).prop('disabled', true);
						...
						$('input[name=txtCustName]').val(val.CustName).prop('disabled',false);
		            							
		            	//Load Contact To
						$('select[name=selContactPerson]').html('<option>Loading...</option>');
						$('select[name=selContactPerson]').load('../../proc/views/SI/vw_contactlist.php?cardcode=' + val.CardCode, function () {
							$('select[name=selContactPerson]').val(val.CntctCode).prop('disabled',false);
							})
	...
	$('#BPModal').on('shown.bs.modal',function(){
		...
		$('#BPCont').load('../../proc/views/SI/vw_bplist.php?CardType=C',function(){
			//Add Scroll Function 
		        ...
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/SI/vw_bplist-load.php',
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){
	...
	$(document.body).on('dblclick','#tblBP tbody > tr',function(){
		...
		$('select[name=selContactPerson]').load('../../proc/views/SI/vw_contactlist.php?cardcode=' + encodeURI(BPCode));
		...
		//Load Ship To
		$('select[name=selShipTo]').html('<option>Loading...</option>');
		$('select[name=selShipTo]').load('../../proc/views/SI/vw_sel_shipto.php?cardcode=' + encodeURI(BPCode));
		
		//Load Bill To
		$('select[name=selBillTo]').html('<option>Loading...</option>');
		$('select[name=selBillTo]').load('../../proc/views/SI/vw_sel_billto.php?cardcode=' + encodeURI(BPCode));
		
		//Load Ship To
		$('textarea[name=txtShipTo]').html('Loading...');
		$('textarea[name=txtShipTo]').load('../../proc/views/SI/vw_shipto.php?cardcode=' + encodeURI(BPCode));
		
		//Load Bill To
		$('textarea[name=txtBillTo]').html('Loading...');
		$('textarea[name=txtBillTo]').load('../../proc/views/SI/vw_billto.php?cardcode=' + encodeURI(BPCode));
	...
	$(document.body).on('keyup','input[name=BPSearch]',function(){
        ...
        $('#BPCont table tbody').load('../../proc/views/SI/vw_bplist-load.php?srchval=' + encodeURI(searchVal));
	})
	//End Search BP
	
	//Address Change
	$(document.body).on('change','select[name=selShipTo]',function(){
		
		var AddressId = $(this).val();
		var CardCode = $('input[name=txtVendor]').val();
		
		
		//Load Ship To
		$('textarea[name=txtShipTo]').html('Loading...');
		$('textarea[name=txtShipTo]').load('../../proc/views/SI/vw_shipto-load.php?cardcode=' + CardCode +'&addressid='+ AddressId);
		
	})
	//End Service Type Change
	
	//Address Change
	$(document.body).on('change','select[name=selBillTo]',function(){
		...
		//Load Bill To
		$('textarea[name=txtBillTo]').html('Loading...');
		$('textarea[name=txtBillTo]').load('../../proc/views/SI/vw_billto-load.php?cardcode=' + CardCode +'&addressid='+ AddressId);
	...
	$(document.body).on('keyup','input[name=txtBaseEntry]',function()
	{
		...
		if(basentry != ''){
			...
			$.getJSON('../../proc/views/SI/vw_getdrdata.php?docentry=' + basentry, function(data) {
	            /* data will hold the php array as a javascript object */
           		...
           		$.each(data, function(key, val) {
	            	...
	            	if(val.DocStatus == 'C'){
		            	...
		            	$('textarea[name=txtRemarksF]').val(val.Comments + ' Based on Delivery ' + val.DocNum + '.').prop('disabled',true);
						...
						$('textarea[name=txtRemarksF]').val(val.Comments + ' Based on Delivery ' + val.DocNum + '.').prop('disabled',false);
						$('select[name=selShipTo]').html('<option>Loading...</option>');
						$('select[name=selShipTo]').load('../../proc/views/SI/vw_sel_shipto.php?cardcode=' + val.CardCode, function () {
							$('select[name=selShipTo]').val(val.ShipToCode).prop('disabled',true);
							})
	            ...
	            setTimeout(function(){
	            	$('#modal-load-init').modal('hide');
	            	$('#modal-load-init').modal('show');
	            	populatedetDR(basentry,function(){
            			//$('input[name=TotBefDisc]').trigger('keyup')
		            	$('#modal-load-init').modal('hide');
	...
	$(document.body).on('keyup','input[name=txtBaseEntry1]',function()
	{
		var basentry = $(this).val();
		...
		if(basentry != ''){
			//Show Base Document
			//Get PR data using JSON
			$.getJSON('../../proc/views/SI/vw_getsodata.php?docentry=' + basentry, function(data) {
	            /* data will hold the php array as a javascript object */
                  ...
                  $('select[name=txtU_SalesCoordinator]').val(val.U_SalesCoordinator).prop('disabled', true);
		            	...
		            	$('textarea[name=txtRemarksF]').val(val.Comments + ' Based on Sales Order ' + val.DocNum + '.').prop('disabled',true);
                  ...
                  // UDFs
                  $('input[name=txtU_ContrctPrice]').val(val.U_ContrctPrice).prop('disabled',false);
                  $('input[name=txtU_Downpaymnt]').val(val.U_Downpaymnt).prop('disabled',false);
                  $('input[name=txtU_MiscFee]').val(val.U_MiscFee).prop('disabled',false);
                  $('input[name=txtU_ResrvationFee]').val(val.U_ResrvationFee).prop('disabled',false);
                  $('select[name=txtU_Realty]').val(val.U_Realty).prop('disabled', false);
                  $('select[name=txtU_SalesCoordinator]').val(val.U_SalesCoordinator).prop('disabled', false);
						...
						$('textarea[name=txtRemarksF]').val(val.Comments + ' Based on Sales Order ' + val.DocNum + '.').prop('disabled',false);
						$('select[name=selShipTo]').html('<option>Loading...</option>');
						$('select[name=selShipTo]').load('../../proc/views/SI/vw_sel_shipto.php?cardcode=' + val.CardCode, function () {
							$('select[name=selShipTo]').val(val.ShipToCode).prop('disabled',true);
							})
	            ...
	            setTimeout(function(){
	            	$('#modal-load-init').modal('hide');
	            	$('#modal-load-init').modal('show');
	            	populatedetSO(basentry,function(){
            			//$('input[name=TotBefDisc]').trigger('keyup')
		            	$('#modal-load-init').modal('hide');
	...
	$(document.body).on('click','#btnCopy > li > a', function () {
  		...
  		/* if(href == '#DR'){
  			//Copy To
  			window.open("../DR/DR.php?BaseEntry=" + docentry, "", "width=1130,height=550,left=220,top=110");
  			//End Copy To
  		} */
	});
	
	$(document.body).on('click','#btnAddCustomer', function () {
  		window.open("../BP/BP.php", "", "");
	});
	...
	{
		$('#SOCont').html('<p class="text-center"><img src=../../img/ajax-loader.gif /> Loading...</p>');
        var vendor = $('input[name=txtVendor]').val();
		
        $('#SOCont').load('../../proc/views/SI/vw_drdoclist.php?vendor=' + encodeURI(vendor), function () {
            //Add Scroll Function 
            $('#SOCont .table-responsive').bind('scroll', function () {
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        var itemcode = $('#SOCont table tbody > tr:last').children('td').eq(0).text();
                        var ctr = 0;
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SI/vw_drdoclist-load.php?vendor=' + encodeURI(vendor),
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {

                                    $('#SOCont table tbody').append(html);
                                    $('#itm-loader').each(function () {
                                        $(this).remove();
                                    });
        ...
        $('input[name=DRSearch]').focus();
    })
    //End Load PO Documents
	
	$('#SO1Modal').on('shown.bs.modal', function () 
	{
		$('#SO1Cont').html('<p class="text-center"><img src=../../img/ajax-loader.gif /> Loading...</p>');
        var vendor = $('input[name=txtVendor]').val();
		
        $('#SO1Cont').load('../../proc/views/SI/vw_sodoclist.php?vendor=' + encodeURI(vendor), function () {
            //Add Scroll Function 
            $('#SO1Cont .table-responsive').bind('scroll', function () {
                if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20) {
                    if ($(this).scrollTop() > 0) {
                        var itemcode = $('#SO1Cont table tbody > tr:last').children('td').eq(0).text();
                        var ctr = 0;
                        ...
                        if (ctr == 0) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '../../proc/views/SI/vw_sodoclist-load.php?vendor=' + encodeURI(vendor),
                                data: {
									itemcode : itemcode
								},
                                success: function (html) {

                                    $('#SO1Cont table tbody').append(html);
                                    $('#itm-loader').each(function () {
                                        $(this).remove();
                                    });
        ...
        $('input[name=SOSearch]').focus();
    })
    //End Load PO Documents
	
	$(document.body).on('keyup', 'input[name=DRSearch]', function () {
        var searchVal = $(this).val().toLowerCase();
        var vendor = $('input[name=txtVendor]').val();
		
        $('#SOCont table tbody').html('<tr><td class="text-center" colspan="7"><p><img src=../../img/ajax-loader.gif /></p></td></tr>');
        $('#SOCont table tbody').load('../../proc/views/SI/vw_drdoclist-load.php?srchval=' + encodeURI(searchVal) + '&vendor=' + encodeURI(vendor));
    });
	
	$(document.body).on('keyup', 'input[name=SOSearch]', function () {
        var searchVal = $(this).val().toLowerCase();
        var vendor = $('input[name=txtVendor]').val();
		
        $('#SO1Cont table tbody').html('<tr><td class="text-center" colspan="7"><p><img src=../../img/ajax-loader.gif /></p></td></tr>');
        $('#SO1Cont table tbody').load('../../proc/views/SI/vw_sodoclist-load.php?srchval=' + encodeURI(searchVal) + '&vendor=' + encodeURI(vendor));
    });
	
	$(document.body).on('click', '#tblSODocument tbody > tr', function (e) 
	{
		highlight('#tblSODocument', this);
	});
	
	$(document.body).on('click', '#tblSO1Document tbody > tr', function (e) 
	{
		highlight('#tblSO1Document', this);
	});

	$(document.body).on('dblclick','#tblSODocument tbody > tr',function(){
	...
	$(document.body).on('dblclick','#tblSO1Document tbody > tr',function(){

		var docentry = $(this).children('td.item-1').text();
		
		$('#SO1Modal').modal('hide');
		
		$('input[name=txtBaseEntry1]').val(docentry);
		$('input[name=txtBaseEntry1]').trigger('keyup');
	});
	
	//Print Document
	/* $(document.body).on('click','#btnSelectPrint',function(){
		var docentry = $('input[name=txtDocEntry]').val();
		
		
		if(docentry != ''){
			
			window.open("../../report/SI/si-report.php?docentry=" + encodeURI(docentry), "", "width=1130,height=550,left=220,top=110");
	...
	$(document.body).on('click','#btnSelectPrint > li > a', function () 
	{
		...
		if(href == '#CI')
		{
			window.open("../../report/SI/ci-report.php?docentry=" + encodeURI(docentry), "", "width=1130,height=550,left=220,top=110");
		}			
  });
	...
	$('#EmailModal').on('shown.bs.modal',function(){
		...
		$.ajax({
		type: "POST",
		url: "../../proc/views/SI/vw_email.php",
	...
	//SELECT Email
	$(document.body).on('click', '#btnEmail', function(){
		
		var myEmail = new Array();
		$('input[name="Email[]"]:checked').each(function() {
			myEmail.push($(this).val());
		});
		var myEmailLength = myEmail.length;
		
		$('textarea[name=txtToEmail]').val(myEmail);
	...
	$('#SerialModal').on('shown.bs.modal',function(){
		...
		if(lineno == '')
		{
			$.ajax({
			type: "POST",
			url: "../../proc/views/SI/vw_itemcode-serial.php",
		...
		else
		{
			...
			$.ajax({
			type: "POST",
			url: "../../proc/views/SI/vw_setserial.php",
			data: form_data,
			success: function(data)
				{
	...
	$(document.body).on('click','#btnARListVIEW',function(e)
	{
		$('#resView').empty();
    	...
    	var txtARListFrom = $('input[name=txtARListFrom]').val();
    	var txtARListTo = $('input[name=txtARListTo]').val();
		
		if(err == 0)
		{
			$.ajax({
				type: 'POST',
				url: '../../report/si/rpt_view.php',
				data: {
					txtDateFrom : txtDateFrom,
					txtDateTo : txtDateTo,
					txtARListFrom : txtARListFrom,
					txtARListTo : txtARListTo
				},
				success: function (html) 
				{
					$('#resView').html(html);
	...
	$('#ARListModal').on('shown.bs.modal',function()
	{
		$('#resView').append('<p class="text-center" id="itm-loader"><img src=../../img/ajax-loader.gif /> Loading...</p>');
	...
	$('#ARListModal').on('hide.bs.modal',function()
	{
		$('#resView').empty();
	});
	...
	$(document.body).on('click','#btnARListPDF',function(e)
	{
    	var err = 0;
    	var errmsg = '';
		var txtDateFrom = $('input[name=txtDateFrom]').val();
		...
		if(err == 0)
		{
			window.open("../../report/si/arlist-report.php?txtARListFrom=" + encodeURI(txtARListFrom) + "&txtARListTo=" + encodeURI(txtARListTo) + "&txtDateFrom=" + encodeURI(txtDateFrom) + "&txtDateTo=" + encodeURI(txtDateTo), "", "width=1130,height=550,left=220,top=110");
		}
		else
		{
	...
	$(document.body).on('click','#btnARListEXCEL',function(e)
	{
    	var err = 0;
    	var errmsg = '';
		var txtDateFrom = $('input[name=txtDateFrom]').val();
		...
		if(err == 0)
		{
			window.open("../../report/si/arlist-excel.php?txtARListFrom=" + encodeURI(txtARListFrom) + "&txtARListTo=" + encodeURI(txtARListTo) + "&txtDateFrom=" + encodeURI(txtDateFrom) + "&txtDateTo=" + encodeURI(txtDateTo), "", "width=1130,height=550,left=220,top=110");
		}
		else
		{
			notie.alert(3, errmsg, 2);
	...
	//Save PO
    $(document.body).on('click','#btnSaveInv',function(e){
    	
    	var err = 0;
    	var errmsg = '';
    	var basentry = $('input[name=txtBaseEntry]').val();
    	var basentry1 = $('input[name=txtBaseEntry1]').val();
    	var vendor = $('input[name=txtVendor]').val();
    	var contactperson = $('input[name=txtCustName]').val();
      ...
      // DownPayment Details
      var DPDocEntry = $('input[name=txtDPDocEntry]').val();
      var DpmAmnt = $('input[name=txtDpmAmnt]').val();
      var DpmAppVatBal = $('input[name=txtDpmAppVatBal]').val();
      var DpmGrossBal = $('input[name=txtDpmGrossBal]').val();
		...
		var sel_shipto = $('select[name=selShipTo]').val();
		var billto = $('textarea[name=txtBillTo]').val();
		var sel_billto = $('select[name=selBillTo]').val();

		// UDF HEADER
		var U_ContrctPrice = $('input[name=txtU_ContrctPrice]').val();
		var U_Downpaymnt = $('input[name=txtU_Downpaymnt]').val();
		var U_MiscFee = $('input[name=txtU_MiscFee]').val();
		var U_ResrvationFee = $('input[name=txtU_ResrvationFee]').val();
		var U_Realty = $('select[name=txtU_Realty]').val();
		var U_SalesCoordinator = $('select[name=txtU_SalesCoordinator]').val();
		...
		var tbl2 = $('#tblDetails tbody tr').each(function(i) {  
		        ...
		        if(servicetype == 'I'){
		        	...
		        	if($(this).find('td.ftext').text() == 'N'){
						...
						itArr.push('"' + ($(this).find('input.itemcode').attr('aria-isFather') == undefined? 'Y' : $(this).find('input.itemcode').attr('aria-isFather')) + '"');
						itArr.push('"' + $(this).find('input.serialno').val() + '"');
						itArr.push('"' + $(this).find('input.baseprice').val().replace(/,/g,'') + '"');
						itArr.push('"' + $(this).find('input.barcode').val().replace(/,/g,'') + '"');
		...
		if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SI/exec-savesi.php',
                data: {
                		json : json.replace(/(\r\n|\n|\r)/gm, ''),
                		basentry : basentry,
                		basentry1 : basentry1,
						...
						shipto : shipto,
						sel_shipto : sel_shipto,
						billto : billto,
						sel_billto : sel_billto,
						// Downpayment Details
						DPDocEntry : DPDocEntry,
						DpmAmnt : DpmAmnt, 
						DpmAppVatBal : DpmAppVatBal, 
						DpmGrossBal : DpmGrossBal,
						// udf
						U_ContrctPrice : U_ContrctPrice,
						U_Downpaymnt : U_Downpaymnt,
						U_MiscFee : U_MiscFee,
						U_ResrvationFee : U_ResrvationFee, 
						U_Realty : U_Realty, 
						U_SalesCoordinator : U_SalesCoordinator, 
                ...
                success: function(html){
					...
					if(res[0] == 'true')
					{
						...
						//Refresh the page
						/* $('input[name=txtDocEntry]').val(res[2]);
						$('#btnSave').addClass('hidden');
						$('#btnUpdate').addClass('hidden');
						$('#btnCopyFrom').addClass('hidden');
						$('#btnCopyFromHide').addClass('hidden');
						...
						disablebuttons(true);
						setTimeout(function(){
							location.replace('../../forms/SI/SI.php');
						},1000);
	...
	$(document.body).on('click','#btnSaveInv1',function(e)
	{
	...
	$(document.body).on('click','#btnAddContact',function(e){
    	
		var err = 0;
    	var errmsg = '';
    	var contactperson = $('input[name=txtBPContactPerson]').val();
    	var vendor = $('input[name=txtVendor]').val();
		...
		if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SI/exec-savecontact.php',
                data: {
    ...
    $(document.body).on('click','#btnUpdate',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SI/exec-updatesi.php',
                data: {
                		contactperson : contactperson,
						numatcard : numatcard,
	...
	//Canceled SO
    //Canceled SO
    $(document.body).on('click','#btnCancelInv',function(e){
    	...
    	if(err == 0){
    		...
    		$.ajax({
                type: 'POST',
                url: '../../proc/exec/SI/exec-cancelsi.php',
                data: {
                	docentry : docentry
				},
                success: function(html){
    ...
    //Load DP LIST
	$('#DPModal').on('shown.bs.modal',function(){
    var vendor = $('input[name=txtVendor]').val();
    var selDocCur = $('select[name=selDocCur]').val(); 
    var txtTotalPaymentDue = $('input[name=txtTotalPaymentDue]').val();
     
    if(vendor == ''){
      //Alert when error
      notie.alert(3, 'Select Customer', 3);
      //End
    }
    if(selDocCur == '##'){
      //Alert when error
      notie.alert(3, 'Select Currency', 3);
      //End
    }
    if(txtTotalPaymentDue == '' || txtTotalPaymentDue == 0){
      //Alert when error
      // $('#DPModal').modal('hide');
      // notie.alert(3, 'Add Item/Service First', 3);
      //End
    }
     
		$('#DPCont').html('<p class="text-center"><img src=../../img/ajax-loader.gif /> Loading...</p>');
		$('#DPCont').load('../../proc/views/SI/vw_dplist.php?CardType=C&vendor=' + encodeURI(vendor) + '&selDocCur=' + encodeURI(selDocCur),function(){
			//Add Scroll Function 
		    $('#DPCont .table-responsive').bind('scroll', function(){
		        if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
		              if($(this).scrollTop() > 0){
		                var itemcode = $('#DPCont table tbody > tr:last').children('td').eq(0).text();
		                var ctr = 0;
		                ...
		                if(ctr == 0){
		                  ...
		                  $.ajax({
		                        type: 'POST',
		                        url: '../../proc/views/SI/vw_dplist-load.php?CardType=C&vendor=' + encodeURI(vendor) + '&selDocCur=' + encodeURI(selDocCur),
		                        data: {
									itemcode : itemcode
								},
		                        success: function(html){

		                          $('#DPCont table tbody').append(html);                
		                          $('#itm-loader').each(function () {
		                            $(this).remove();   
		                          });
		...
		$('input[name=DPSearch]').focus();
	})
	//End Load DP LIST

  //Clear DP DATA
	$('#DPModal').on('hide.bs.modal',function(){
		$('#DPCont').empty();

	})
	//End Clear DP DATA

  //Add Keypress on DP MOdal
	$('#DPModal').keydown(function(e) {
      switch(e.which) {
        case 13: //Enter
          $('tr.selected-whs').trigger('dblclick');
        break;
          ...
          case 40: // down
            ...
            //Check if selected
            if($('#tblDP tbody').find('tr.selected-whs').index() >= 0){
              $('tr.selected-whs').next().trigger('click');

              //$('#WhsCont > .table-responsive').scrollTop(10);
            }else{
              $('#tblDP tbody > tr:first').trigger('click');
            }
            //End
          break;
  ...
  //End Add Keypress on DP Modal

  //Highlight Item Table Row Click
	$(document.body).on('click','#tblDP tbody > tr',function(e){

		
		highlight('#tblDP',this);
	...
	//Select DP Table Row Click
	$(document.body).on('dblclick','#tblDP tbody > tr',function(){
    var DocEntry = $(this).children('td.item-1').text();
		var DpmAmntBal = $(this).children('td.item-5').find('input').val();
		var DpmAppVatBal = $(this).children('td.item-6').find('input').val();; 
		var DpmGrossBal = $(this).children('td.item-7').find('input').val();;

    
    $('input[name=txtDpmAppVatBal]').val(DpmAppVatBal);
    $('input[name=txtDpmGrossBal]').val(DpmGrossBal);
    $('input[name=txtDPDocEntry]').val(DocEntry);
    $('input[name=txtDpmAmnt]').trigger('keyup').val(DpmAmntBal);
		
		 
		$('#btnDpRemove').removeClass('hidden');
    $('#btnDpModal').addClass('hidden');
		$('#DPModal').modal('hide');
		
	})
	//End Select DP Table Row Click

	//Search DP
	$(document.body).on('keyup','input[name=DPSearch]',function(){
    var vendor = $('input[name=txtVendor]').val();
    var selDocCur = $('select[name=selDocCur]').val(); 
		
		var searchVal = $(this).val().toLowerCase();
        $('#DPModal table tbody').html('<tr><td class="text-center" colspan="7"><p><img src=../../img/ajax-loader.gif /></p></td></tr>');
        $('#DPModal table tbody').load('../../proc/views/SI/vw_dplist-load.php?srchval=' + encodeURI(searchVal)+ '&vendor=' + encodeURI(vendor) + '&selDocCur=' + encodeURI(selDocCur));
	})
	//End Search DP

  //Remove DP Button Click
	$(document.body).on('click','#btnDpRemove',function(e){ 

    
    $('input[name=txtDpmAppVatBal]').val(0.00);
    $('input[name=txtDpmGrossBal]').val(0.00);
    $('input[name=txtDPDocEntry]').val('');
    $('input[name=txtDpmAmnt]').trigger('keyup').val(0.00);

    $('#btnDpRemove').addClass('hidden');
    $('#btnDpModal').removeClass('hidden');

	})
	//End Remove DP Button Click

  // OnChange of DP Amount
  $(document.body).on('keyup','input[name=txtDpmAmnt]',function(){
    // tax
    totaltax = $('input[name=txtTaxF]').val();
    downpaymenttax = $('input[name=txtDpmAppVatBal]').val();
    // total
    totalamt = $('input[name=txtTotalPaymentDue]').val();
    downpaymentTotal = $('input[name=txtDpmGrossBal]').val();
 
    // $('input[name=txtTaxFWithDp]').val(0);
    // $('input[name=txtTotalPaymentDueWithDp]').val(0);
    $('input[name=txtTaxFWithDp]').val(computeTaxAmtFooterWithDownpayment(totaltax, downpaymenttax));
    $('input[name=txtTotalPaymentDueWithDp]').val(computeTPaymentDueWithDownpayment(totalamt, downpaymentTotal));
    
     
	})
  // End OnChange of DP Amount

    

  // Realty
  $('select[name=txtU_Realty]').html('<option>Loading...</option>');
	$('select[name=txtU_Realty]').load('../../proc/views/SO/vw_udt.php?udt=REALTY');
  // End Realty

  // Sales Coordinator
  $('select[name=txtU_SalesCoordinator]').html('<option>Loading...</option>');
	$('select[name=txtU_SalesCoordinator]').load('../../proc/views/SO/vw_udt.php?udt=SALESCOORDINATOR');
  // End Sales Coordinator
	...
	function checkitemcode(itemcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SI/vw_checkitemcode.php',
	        async: false,
	        data: {
				itemcode : itemcode
			},
	...
	function checkserialno(serialno){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SI/vw_checkserial.php',
	        async: false,
	        data: 'itemcode=' + serialno,
	        success: function(html){
	...
	function checkwhs(whscode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SI/vw_checkwhscode.php',
	        async: false,
	        data: {
				whscode : whscode
			},
	...
	function checkacctcode(acctcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SI/vw_checkacctcode.php',
	        async: false,
	        data: {
				acctcode : acctcode
			},
	...
	function checkbpcode(bpcode){
	  var result = '';
	   ...
	   $.ajax({
	        type: 'POST',
	        url: '../../proc/views/SI/vw_checkbpcode.php',
	        async: false,
	        data: {
				bpcode : bpcode
			},
    ...
    // with DP 
    $('input[name=txtDpmAmnt]').trigger('keyup');
  ...
  // Compute Tax Amount with Downpayment Footer 
	function computeTaxAmtFooterWithDownpayment(totaltax,downpaymenttax){
	  totaltax = isNaN(parseFloat(totaltax.replace(/,/g,'')))? 0: parseFloat(totaltax.replace(/,/g,''));
	  downpaymenttax = isNaN(parseFloat(downpaymenttax.replace(/,/g,'')))? 0 : parseFloat(downpaymenttax.replace(/,/g,''));
	  var total = totaltax - downpaymenttax;
	  return formatMoney(total);
	}
	// Compute Tax Amount with Downpayment Footer

  // Compute Total Amount with Downpayment Footer  
	function computeTPaymentDueWithDownpayment(totalAmt,downpaymentTotal){
	  totalAmt = isNaN(parseFloat(totalAmt.replace(/,/g,'')))? 0 : parseFloat(totalAmt.replace(/,/g,'')); 
	  downpaymentTotal = isNaN(parseFloat(downpaymentTotal.replace(/,/g,'')))? 0 : parseFloat(downpaymentTotal.replace(/,/g,''));

	  var total = totalAmt - downpaymentTotal;
	  return formatMoney(total);
	}
	// End Compute Total Amount with Downpayment Footer 
	...
	function populatedet(docentry,callback){
		...
		$('#tblDetails tbody').load('../../proc/views/SI/vw_documentdetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetDR(docentry,callback){

			
		$('#tblDetails tbody').load('../../proc/views/SI/vw_drdetailsdata.php?docentry=' + docentry,function(result){
	...
	function populatedetSO(docentry,callback){

			
		$('#tblDetails tbody').load('../../proc/views/SI/vw_sodetailsdata.php?docentry=' + docentry,function(result){

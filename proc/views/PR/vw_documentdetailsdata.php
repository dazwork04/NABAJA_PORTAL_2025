...
$qry = odbc_exec($MSSQL_CONN, " 
			SELECT T0.DocType,'N' AS LineType,T1.ItemCode,T1.Dscription
                                ,(T1.Quantity) AS Quantity,T2.InvntryUom,T1.PriceBefDi,T1.WhsCode
                                ,T1.VatGroup,T1.DiscPrcnt,T1.PriceAfVAT,
								CASE WHEN T1.Currency = 'PHP' THEN T1.VatSum ELSE T1.Quantity * T1.Price END as VatSum,
								CASE WHEN T1.Currency = 'PHP' THEN T1.LineTotal ELSE T1.Quantity * T1.Price END as LineTotal,
								CASE WHEN T1.Currency = 'PHP' THEN T1.GTotal ELSE T1.Quantity * T1.Price END as GTotal,
								'' AS LineText,T3.AcctCode,T3.FormatCode,T3.AcctName
                                ,T1.LineNum,T1.LineStatus,'A' AS SubSeq,T1.LineNum AS AftLineNum
                                , T1.OcrCode AS OcrCode
                                , T1.OcrCode2 AS OcrCode2, T1.unitMsr2 as unitMsr2, T1.Currency as Currency, T1.Text
			...
			UNION ALL
			SELECT T0.DocType,'Y' AS LineType,'' AS ItemCode,'' AS Dscription
                        ...
                        ,'B' AS SubSeq,T1.AftLineNum, '' AS OcrCode, '' as unitMsr2, '' as Currency, '' AS Text
                        , '' AS OcrCode2
				FROM [" . $_SESSION['mssqldb'] . "].[dbo].OPRQ T0
...
while (odbc_fetch_row($qry)) {
    ...
    $ItemDetails = odbc_result($qry, "Text");

    $BarCode = '';
    $unitMsr2 = odbc_result($qry, "unitMsr2");
	...
	$Currency .= '<option value="'.odbc_result($qry, "Currency").'" selected>'.odbc_result($qry, "Currency").'</option>';
	//Free Result
				...
				<tr>
					...
					<td style="padding-top: 2px;  padding-bottom: 2px;">
						<div class="input-group itemcodeCont" style="height: 18px; padding: 0 4px; margin: 0;">
							...
							<span style="height: 18px; padding: 0 4px; margin: 0;" class="input-group-addon" data-toggle="modal" data-target="#ItemModal"><span class="glyphicon glyphicon-list"></span></span>
						</div>
					...
					<td>
						<div class="input-group warehouseCont" style="height: 18px; padding: 0 4px; margin: 0;">
							<input ' . $disabled . ' class="form-control input-sm warehouse" value="' . $Whse . '" />
					...
					<td class="hidden"><input readonly class="form-control input-sm uomname" value="' . $unitMsr2 . '"></td>
					<td class="hidden"><select class="form-control input-sm currency">"' .$Currency. '"</select></td>
					...
					<td><input ' . $disabled . ' class="form-control input-sm itemdetails" value="' . $ItemDetails . '" maxlength="1000"></td>

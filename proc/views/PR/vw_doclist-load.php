...
$OwnerCode = $_SESSION['SESS_EMP'];
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 20 T0.DocEntry,T0.DocNum, T0.Comments,T0.DocDate, T0.OwnerCode,
												T0.NumAtCard,T0.DocStatus, T0.ReqName, T0.CANCELED
												FROM [OPRQ] T0
												WHERE (T0.ReqName LIKE '%$srchval%' OR T0.DocNum LIKE '%$srchval%' OR T0.Comments LIKE '%$srchval%' 
												OR CONVERT(VARCHAR(10),T0.DocDate,101) LIKE '%$srchval%' OR T0.NumAtCard LIKE '%$srchval%')
	...
	while (odbc_fetch_row($qry)) {
		...
		echo '<tr class="srch">
					...
					<td class="item-7">'.odbc_result($qry, 'ReqName').'</td>

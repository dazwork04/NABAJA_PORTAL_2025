...
if(isset($_GET['srchval'])){
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 100 ItemCode, ItemName, OnHand, InvntryUom, BuyUnitMsr, ManBtchNum, NumInBuy FROM OITM WHERE validFor = 'Y' AND (PrchseItem = 'Y' OR InvntItem = 'Y') AND (ItemCode LIKE '%".$srchval."%' OR ItemName LIKE '%".$srchval."%') ORDER BY ItemCode,ItemName";
}else{
	$itemcode = $_POST['itemcode'];
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 50 ItemCode, ItemName, OnHand, InvntryUom, BuyUnitMsr, ManBtchNum, NumInBuy FROM OITM WHERE validFor = 'Y' AND (PrchseItem = 'Y' OR InvntItem = 'Y') AND ItemCode > '".$itemcode."' ORDER BY ItemCode,ItemName";	
}

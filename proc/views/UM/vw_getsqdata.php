...
$qry = odbc_exec($MSSQL_CONN, "USE [".$_SESSION['mssqldb']."]; 
		SELECT T0.DocEntry,T0.DocNum,T0.DocType,T0.CardCode,T0.CardName,T1.Name,T0.NumAtCard,T0.GroupNum,T0.Series,T2.SeriesName,T2.BPLId,T0.TaxDate,T0.DocDueDate,T0.DocDate,T0.Comments,T0.DocStatus,T0.DocTotal,T0.DiscPrcnt,T0.DiscSum,T0.VatSum,(T0.DocTotal - T0.VatSum + T0.DiscSum) AS TotBefDisc 
		FROM OQUT T0
		LEFT JOIN OCPR T1

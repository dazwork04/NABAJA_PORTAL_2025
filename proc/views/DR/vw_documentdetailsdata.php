...
$qry = odbc_exec($MSSQL_CONN, " 
			SELECT T0.DocType,'N' AS LineType,T1.ItemCode,T1.Dscription,T2.InvntryUom,(T1.Quantity) AS Quantity,T1.OpenSum,T2.InvntryUom,T1.PriceBefDi,T1.WhsCode,T1.VatGroup,T1.DiscPrcnt,T1.PriceAfVAT,
		...
		INNER JOIN [".$_SESSION['mssqldb']."].[dbo].DLN1 T1 ON T0.DocEntry = T1.DocEntry
		...
		LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OPRC] T6 ON T1.OcrCode3 = T6.PrcCode
		LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OPRC] T7 ON T1.OcrCode4 = T7.PrcCode

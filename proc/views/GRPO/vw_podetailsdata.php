...
$qry = odbc_exec($MSSQL_CONN, "USE [".$_SESSION['mssqldb']."]; 
			SELECT T0.DocType,'N' AS LineType,T1.ItemCode,T1.Dscription,T2.InvntryUom,(T1.OpenQty) AS Quantity,T1.OpenSum,T2.InvntryUom,T1.PriceBefDi,T1.WhsCode,T1.VatGroup,T1.DiscPrcnt,T1.PriceAfVAT,
				...
				T3.AcctCode,T3.FormatCode,T3.AcctName,T1.LineNum,T1.LineStatus,'A' AS SubSeq,T1.LineNum AS AftLineNum, '' AS Serial, 
				T1.OcrCode,
			...
			LEFT JOIN [OPRC] T4 ON T1.OcrCode = T4.PrcCode
			...
			WHERE T0.DocEntry ='$docentry' AND T1.LineStatus = 'O'
			UNION ALL
			SELECT T0.DocType,'Y' AS LineType,'' AS ItemCode,'' AS Dscription,'' AS InvntryUom,0 AS Quantity,0 AS OpenSum,'' AS InvntryUom,0 AS PriceBefDi,'' AS WhsCode,'' AS VatGroup,0 AS DiscPrcnt,0 AS PriceAfVAT,0 AS VatSum,0 AS LineTotal,0 AS GTotal, 	
				'' AS AcctCode,'' AS FormatCode,'' AS AcctName,T1.LineSeq AS LineNum,'' AS LineStatus,'B' AS SubSeq,T1.AftLineNum, 'NULL' AS Serial, 
				'' AS OcrCode,
				'' AS DeptName,
				'' AS Project,
				'' AS PrjName,
			...
			FROM [OPOR] T0
			INNER JOIN [POR10] T1 ON T0.DocEntry = T1.DocEntry
		...
		if($Ftext == 'N'){
			echo '
				<tr>
					...
					<td>
						<input '.$disabled.' class="form-control input-sm discount numeric" value="'.$Discount.'">
					</td>
					<td>
						<input '.$disabled.' class="form-control input-sm grossprice numeric" value="'.$GrossPrice.'">
					</td>
					<td class="hidden"><input readonly class="form-control input-sm taxamount numeric" value="'.$TaxAmt.'"></td>
					<td><input readonly class="form-control input-sm linetotal numeric" value="'.$LineTotal.'">
						</td>

...
$qry = odbc_exec($MSSQL_CONN, " 
			...
			'' AS LineText,T3.AcctCode,T3.FormatCode,T3.AcctName,T1.LineNum,T1.LineStatus,'A' AS SubSeq,
			T1.WtLiable,
			T1.OcrCode,
			...
			T1.LineNum AS AftLineNum, T1.Text,
			'' AS Serial
				
				FROM [".$_SESSION['mssqldb']."].[dbo].[OPCH] T0
				INNER JOIN [".$_SESSION['mssqldb']."].[dbo].[PCH1] T1 ON T0.DocEntry = T1.DocEntry
				LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OITM] T2 ON T1.ItemCode = T2.ItemCode
				LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OACT] T3 ON T1.AcctCode = T3.AcctCode
				LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OPRC] T4 ON T1.OcrCode = T4.PrcCode
				LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OPRJ] T5 ON T1.Project = T5.PrjCode
				LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OPRC] T6 ON T1.OcrCode2 = T6.PrcCode
				LEFT JOIN [".$_SESSION['mssqldb']."].[dbo].[OPRC] T7 ON T1.OcrCode3 = T7.PrcCode
				WHERE T0.DocEntry ='$docentry' 
				AND T1.LineStatus = 'O'
			UNION ALL
			SELECT T0.DocType,'Y' AS LineType,'' AS ItemCode,'' AS Dscription,'' AS InvntryUom,0 AS Quantity,0 AS OpenSum,'' AS InvntryUom,0 AS PriceBefDi,'' AS WhsCode,'' AS VatGroup,0 AS DiscPrcnt,0 AS PriceAfVAT,0 AS VatSum,0 AS LineTotal,0 AS GTotal,T1.LineText,'' AS AcctCode,'' AS FormatCode,'' AS AcctName,
			'' AS WtLiable,
			'' AS OcrCode,
			'' AS DeptName,
			'' AS Project,
			...
			'' AS OcrCode3,
			'' AS EquipName,
				T1.LineSeq AS LineNum,'' AS LineStatus,'B' AS SubSeq,T1.AftLineNum, '' AS Text, 'NULL' AS Serial
				FROM [".$_SESSION['mssqldb']."].[dbo].[OPCH] T0
				INNER JOIN [".$_SESSION['mssqldb']."].[dbo].[PCH10] T1
		...
		if($Ftext == 'N'){
			echo '
				<tr>
					...
					<td><input '.$disabled.' class="form-control input-sm itemdetails" maxlength="1000" value="'.$Text.'"></td>
					<td class="hidden"><input readonly class="form-control input-sm grosstotal numeric" value="'.$GrossTotal.'"></td>
					<td>
					<select class="form-control input-sm withholdingtax">
						<option value="1" '.$selectedY.'>Yes</option>

...
$qry = odbc_exec($MSSQL_CONN, "USE [".$_SESSION['mssqldb']."];
		SELECT T0.DocEntry,T0.DocNum,T0.DocType,T0.CardCode,T0.CardName,T1.Name,T0.NumAtCard,T0.GroupNum,T0.Series,T2.SeriesName,'' AS BPLId,T0.TaxDate,T0.DocDueDate,T0.DocDate,T0.Comments,T0.DocStatus,T0.DiscPrcnt, T0.ShipToCode, T0.PayToCode,
		CASE WHEN T0.DocCur = 'PHP' THEN T0.DocTotal ELSE T0.DocTotalFC END AS DocTotal,
			...
			END AS TotBefDisc,
		T0.SlpCode, T0.OwnerCode, T3.lastname + ', ' + T3.firstName AS employeename, T4.Currency, T0.CurSource, T0.DocCur, T0.DocRate, T0.Address, T0.Address2, T0.CANCELED, T4.ListNum,
		T0.CntctCode,
    ...
    T0.U_SalesCoordinator,
		...
		CASE WHEN T1.Name IS NULL THEN '' ELSE T1.Name END AS ContactName
		FROM ORDR T0
		LEFT JOIN OCPR T1
...
while (odbc_fetch_row($qry)) {
	...
	$arr[] = array("DocNum" => odbc_result($qry, 'DocNum'),
				...
				"ContactName" => utf8_encode(odbc_result($qry, 'ContactName')),
				"Canceled" => $Canceled,

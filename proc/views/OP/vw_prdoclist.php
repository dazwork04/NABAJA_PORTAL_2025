	...
	<table class="table table-condensed table-hover" id="tblPRDocument">
	  ...
	  <tbody>
	  	<?php
	  		...
	  		$qry = odbc_exec($MSSQL_CONN, "USE [".$_SESSION['mssqldb']."]; SELECT T0.DocEntry,T0.DocNum,T0.ReqName,T0.Comments,CONVERT(VARCHAR(10),T0.DocDate,101) AS DocDate,T0.DocStatus 
																			FROM [OPRQ] T0
																			WHERE T0.DocStatus = 'O' AND T0.DocType = '$servicetype'
																			ORDER BY T0.DocEntry");
			while (odbc_fetch_row($qry)) {
				echo '<tr class="srch">
						<td class="item-1 hidden">'.odbc_result($qry, 'DocEntry').'</td>
						<td class="item-2">'.odbc_result($qry, 'DocNum').'</td>
						<td class="item-3">'.odbc_result($qry, 'ReqName').'</td>

...
UNION ALL
SELECT T0.[TransId] AS DocEntry,
		T0.[TransId] AS DocNum,
		T0.[DueDate],
		T0.Ref1,
		CASE WHEN T1.LocManTran = 'Y' THEN 
				CASE WHEN T0.DebCred = 'D' THEN T0.Debit * -1  ELSE T0.Credit END 
			ELSE
				CASE WHEN T0.DebCred = 'C' THEN T0.Credit ELSE T0.Debit END
			END
		AS InsTotal,
		CASE WHEN T1.LocManTran = 'Y' THEN 
				CASE WHEN T0.DebCred = 'D' THEN T0.BalDueDeb * -1  ELSE T0.BalDueCred END 
			ELSE
				CASE WHEN T0.DebCred = 'C' THEN T0.BalDueCred ELSE T0.BalDueDeb END
			END
		AS PaidToDate,
		'OJDT' AS InvType
FROM JDT1 T0
...
AND (BalDueCred != 0 OR BalDueDeb != 0) 
AND T0.TransType != 18
AND ShortName = '$bpcode'");
$ctr = 1;
?>
<?php
while (odbc_fetch_row($qry)):
	...
	$InvType = odbc_result($qry, "InvType"); 
	...
	if($InvType == 'OJDT')
	{
		$balanceDue = number_format(odbc_result($qry, "PaidToDate"), 2); 
	}
	else
	{
    ...
    <tr>
        ...
        <td>
			...
			<input readonly class="form-control input-sm " value="<?php echo $documentNumber1; ?>" />
			<input readonly class="form-control input-sm invtype hidden" value="<?php echo $InvType; ?>" />
		</td>

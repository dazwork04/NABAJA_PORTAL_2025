...
if(isset($_GET['srchval'])){
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 20 T0.DocEntry, 
												...
												AND T0.CardCode = '".$vendor."' AND T0.DocStatus = 'O'
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 20 T0.DocEntry, 
															...
															T0.CANCELED
												FROM [OPDN] T0
												WHERE T0.DocEntry < '".$itemcode."' AND T0.CardCode = '".$vendor."' AND T0.DocStatus = 'O'
												ORDER BY T0.DocEntry DESC";
}
}
else
{
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 20 T0.DocEntry, 
															...
															T0.CANCELED
												FROM [OPDN] T0
												WHERE T0.DocEntry < '".$itemcode."' AND T0.DocStatus = 'O'
												ORDER BY T0.DocEntry DESC";
}
}
...
<?php
	...
	while (odbc_fetch_row($qry)) {
		...
		echo '<tr class="srch">
				...
				<td class="item-4">'.odbc_result($qry, 'NumAtCard').'</td>
				<td class="item-5">'.odbc_result($qry, 'CardName').'</td>
				<td class="item-6">'.odbc_result($qry, 'Comments').'</td>
				<td class="item-7">'.$Canceled.'</td>
		      </tr>';
	}

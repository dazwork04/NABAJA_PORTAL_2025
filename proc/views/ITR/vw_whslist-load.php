...
if(isset($_GET['srchval'])){
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; 
						SELECT WhsCode,WhsName
							FROM OWHS 
							Inactive = 'N' AND			
						(WhsCode LIKE '%".$srchval."%' OR WhsName LIKE '%".$srchval."%') ORDER BY WhsCode";
}else{
	$itemcode = $_POST['itemcode'];
	
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT WhsCode,WhsName
							FROM OWHS 
							WHERE Inactive = 'N' AND WhsCode > '".$itemcode."' ORDER BY WhsCode";	
}

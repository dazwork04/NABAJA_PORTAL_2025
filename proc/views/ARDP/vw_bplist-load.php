...
if(isset($_GET['srchval'])){
	...
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 20 CardCode,CardName,Balance,CntctPrsn, GroupNum, Currency, ListNum FROM OCRD WHERE frozenFor = 'N' AND CardType='C' AND (CardCode LIKE '%".$srchval."%' OR CardName LIKE '%".$srchval."%') ORDER BY CardCode";
}else{
	$cardcode = $_POST['itemcode'];
	
	$itemqry = "USE [".$_SESSION['mssqldb']."]; SELECT TOP 20 CardCode,CardName,Balance,CntctPrsn, GroupNum, Currency, ListNum FROM OCRD WHERE frozenFor = 'N' AND CardType='C' AND CardCode > '".$cardcode."' ORDER BY CardCode";	
}

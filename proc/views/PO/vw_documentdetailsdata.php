...
$qry = odbc_exec($MSSQL_CONN, " USE [".$_SESSION['mssqldb']."];
			SELECT T0.DocType,'N' AS LineType,
					T1.ItemCode,
					T1.Dscription,
					T2.InvntryUom,
					T1.Quantity,
			...
			LEFT JOIN [dbo].[OACT] T3 ON T1.AcctCode = T3.AcctCode
			LEFT JOIN [dbo].[OPRC] T4 ON T1.OcrCode = T4.PrcCode 
			LEFT JOIN [dbo].[OPRJ] T5 ON T1.Project = T5.PrjCode
			LEFT JOIN [dbo].[OPRC] T6 ON T1.OcrCode2 = T6.PrcCode
			LEFT JOIN [dbo].[OPRC] T7 ON T1.OcrCode3 = T7.PrcCode
			LEFT JOIN [dbo].[OACT] T8 ON T1.AcctCode = T8.AcctCode
			WHERE T0.DocEntry = $docentry
			UNION ALL
			SELECT T0.DocType,'Y' AS LineType,
					'' AS ItemCode,
					'' AS Dscription,
					'' AS InvntryUom,
					0 AS Quantity,
					...
					'' AS Text
			FROM  [dbo].[OPOR] T0
			INNER JOIN [dbo].[POR10] T1 ON T0.DocEntry = T1.DocEntry
			WHERE T0.DocEntry = $docentry
...
while (odbc_fetch_row($qry)) 
{
	$ItemCode = utf8_encode(odbc_result($qry, "ItemCode"));
	$ItemName = utf8_encode(odbc_result($qry, "Dscription"));
	$InvntryUom = odbc_result($qry, "InvntryUom");
	$POAcctCode = odbc_result($qry, "POAcctCode");
	...
	$ServiceRemarks = utf8_encode(odbc_result($qry, "Dscription"));
	...
	$Text = utf8_encode(odbc_result($qry, 'Text'));
	...
	if($ServiceType == 'I'){
		if($Ftext == 'N'){
			echo '
				<tr>
					...
					<td class="hidden">
						<input class="form-control input-sm glaccount" value="'.$POAccount.'" readonly/>
						<input type="hidden" class="form-control input-sm acctcode" value="'.$POAcctCode.'" readonly/>
					</td>
					<td style="padding-top: 2px;  padding-bottom: 2px;" ><input '.$disabled.' class="form-control input-sm itemname" value="'.$ItemName.'" maxlength="200"/></td>
					<td style="padding-top: 2px;  padding-bottom: 2px;" ><input '.$disabled.' class="form-control input-sm qty required numericvalidate" value="'.$Quantity.'">
					...
					<td style="padding-top: 2px;  padding-bottom: 2px;" ><input '.$disabled.' class="form-control input-sm price required numeric " value="'.$Price.'"></td>
					<td style="padding-top: 2px;  padding-bottom: 2px;" >
					...
					<td class="hidden"><input '.$disabled.' class="form-control input-sm itemdetails" maxlength="1000" value="'.$Text.'"></td>
            ...
            <div class="input-group departmentCont" style="height: 18px; padding: 0 4px; margin: 0;">
              ...
              <span class="input-group-addon" style="height: 18px; padding: 0 4px; margin: 0;" data-toggle="modal" data-target="#DepartmentModal"><span class="glyphicon glyphicon-list"></span></span>
					...
					<td style="padding-top: 2px;  padding-bottom: 2px;" class="hidden">
						<input readonly class="form-control input-sm lineno" value="'.$LineNum.'">
						<input readonly class="form-control input-sm deleterow" value="Y">
					</td>
		...
		echo '
			<tr>
				...
				<td style="padding-top: 2px;  padding-bottom: 2px;"><textarea '.$disabled.' class="form-control input-sm remarks required">'.$ServiceRemarks.'</textarea></td>
				<td>
          ...
          <div class="input-group departmentCont" style="height: 18px; padding: 0 4px; margin: 0;">
            ...
            <span class="input-group-addon" style="height: 18px; padding: 0 4px; margin: 0;" data-toggle="modal" data-target="#DepartmentModal"><span class="glyphicon glyphicon-list"></span></span>
				...
				<td><input readonly class="form-control input-sm lineno" value="'.$LineNum.'" type="hidden"></td>	
			</tr>';

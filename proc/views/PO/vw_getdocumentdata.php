...
$qry = odbc_exec($MSSQL_CONN, "USE [".$_SESSION['mssqldb']."];
		...
		CASE WHEN T0.DocCur = 'PHP' THEN T0.DocTotal ELSE T0.DocTotalFC END AS DocTotal,
			...
			END AS TotBefDisc, 
		T0.SlpCode, T0.OwnerCode, T3.lastname + ', ' + T3.firstName AS employeename, T4.Currency, T0.CurSource, T0.DocCur, T0.DocRate, T0.Address, 
		T0.Address2, T0.CANCELED, T4.ListNum
		FROM OPOR T0
		LEFT JOIN OCPR T1

...
$fromwarehouse = isset($_POST['fromwarehouse']) ? $_POST['fromwarehouse'] : '';
$towarehouse = isset($_POST['towarehouse']) ? $_POST['towarehouse'] : '';
...
$qry = odbc_exec($MSSQL_CONN, "USE [" . $_SESSION['mssqldb'] . "]; SELECT T0.DocNum,T0.DocStatus FROM OWTQ T0 WHERE T0.DocEntry IN($basentry)");
...
//        $oIt->PickRmrk = $pickandpackremarks;
        ...
        if (json_decode($json) != null) {
            ...
            foreach ($json as $key => $value) {
                ...
                if (isset($value[7]) && $value[7] != '' && isset($value[6]) && $value[6] != '') {
                    $oIt->Lines->BaseEntry = $value[7];
                    $oIt->Lines->BaseLine = $value[6];
...
//						echo "false*zzzz" ;// print_r($oIt, true);
        ...
        if ($retval != 0) {
            ...
            //Get DocNo
            $getDN = odbc_exec($MSSQL_CONN, "USE [" . $_SESSION['mssqldb'] . "]; SELECT DocNum FROM OWTR WHERE DocEntry = '$docentry'");
...
if ($err == 0) {
    ...
    echo 'true*Successfully Added IT ' . $docno;
} else {
    echo 'false*' . $errmsg;
}

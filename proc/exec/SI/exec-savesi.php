...
include_once('../../../config/config.php');
  ...
  // Downpayment Details 
  $DPDocEntry = $_POST['DPDocEntry'];
  $DpmAmnt = $_POST['DpmAmnt'];
  $DpmAppVatBal = $_POST['DpmAppVatBal'];
  $DpmGrossBal = $_POST['DpmGrossBal'];
	...
	else
	{
		...
		// UDFs
		$oSi->UserFields->Fields["U_ContrctPrice"]->Value = $U_ContrctPrice;
		$oSi->UserFields->Fields["U_Downpaymnt"]->Value = $U_Downpaymnt;
		$oSi->UserFields->Fields["U_MiscFee"]->Value = $U_MiscFee;
		$oSi->UserFields->Fields["U_ResrvationFee"]->Value = $U_ResrvationFee; 
    
		$oSi->UserFields->Fields["U_Realty"]->Value = $U_Realty;
		$oSi->UserFields->Fields["U_SalesCoordinator"]->Value = $U_SalesCoordinator;
  ...
  // Linked Downpayment
  if (isset($DPDocEntry) && $DPDocEntry != '') 
  { 
    $dpToDraw = $oSi->DownPaymentsToDraw;
    $dpToDraw->AmountToDraw = $DpmAmnt;
    $dpToDraw->DocEntry = $DPDocEntry;
  }

...
//Variables
	...
	$txtDiscPercentF = $_POST['txtDiscPercentF'] == '' ? 0 : $_POST['txtDiscPercentF'];
	$txtTotalPaymentDue = $_POST['txtTotalPaymentDue'] == '' ? 0 : $_POST['txtTotalPaymentDue'];
	...
	else
	{
		...
		$oGrpo->DocDate = $postingdate;
		$oGrpo->DocDueDate = $deliverydate;
		$oGrpo->TaxDate = $documentdate;
		...
		// $oGrpo->UserFields->Fields["U_SiDr"]->Value = $txtRefNo;
		
		if($txtDiscPercentF != 0)
		{
			$oGrpo->DiscountPercent = 0;
		}
				...
				if($servicetype == 'I')
				{
					...
					if ($basentry != '' && $value[11] != '')
					{
						...
						{ // Check if Free Text
							...
							if($value[15] != '')
							{
								$oGrpo->Lines->ProjectCode = $value[15];
							}
							
							if($value[16] != '')
							{
								$oGrpo->Lines->CostingCode2 = $value[16];
							}

							if($value[17] != '')
							{
								$oGrpo->Lines->CostingCode3 = $value[17];
							}
					...
					if ($basentry != '' && $value[6] != ''){
						...
						if($value[8] != '') //Project
						{
							$oGrpo->Lines->ProjectCode = $value[8];
						}
						
						if($value[9] != '') //Employee
						{
							$oGrpo->Lines->CostingCode2 = $value[9];
						}

						if($value[10] != '') //Equipment
						{
							$oGrpo->Lines->CostingCode3 = $value[10];
						}
		...
		else
		{
			...
			if($txtDiscPercentF != 0)
			{
				$oPo->DocTotal = $txtTotalPaymentDue;
			}
...
if($err == 0){
	...
	echo 'true*Successfully Added GRPO #' . $grpodocentry;
}else{
	echo 'false*' . $errmsg;
}

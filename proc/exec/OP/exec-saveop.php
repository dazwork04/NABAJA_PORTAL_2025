...
if ($err == 0) {
	...
	{
        $oRct = $vCmp->GetBusinessObject(46);
        ...
        if (isset($totalcheck) && $totalcheck > 0) 
		{
			...
			$oRct->Checks->AccounttNum = $checkaccount;
		...
		if ($jsoncredit != '') 
		{
			foreach ($jsoncredit as $key => $value1) 
			{
				...
				$oRct->CreditCards->FirstPaymentSum = $value1[4];
        ...
        if (json_decode($json) != null) 
		{
			...
			if($radCategory == 'Vendor')
			{
				foreach ($json as $key => $value) 
				{
					...
					if($value[6] == 'OJDT')
					{
						$oRct->Invoices->InvoiceType  = 30;
					}
					/* else
					{
						$oRct->Invoices->InvoiceType  = 18;
					} */
			...
			else
			{
				foreach ($json as $key => $value) 
				{
					...
					$oRct->AccountPayments->VatAmount = $value[7];
